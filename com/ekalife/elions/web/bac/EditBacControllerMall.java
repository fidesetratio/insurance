 ...
 */
public class EditBacControllerMall extends ParentWizardController {
...
//			refData.put("success", "Berhasil Update Tanggal Terima Admin");
			...
			if(!currentUser.getCab_bank().equals("") && !currentUser.getName().contains("SIMASPRIMA")) {
				...
				refData.put("select_relasi",DroplistManager.getInstance().get("RELATION_MALL.xml","",request));
		...
		/** Halaman 2 : Tertanggung Polis */
			...
			// Andhika - Flag_dis = flag disable rider
			String spaj = editBac.getPemegang().getReg_spaj();
			
			if(spaj==null)spaj="";
			
			if(spaj.equalsIgnoreCase("")){
				editBac.getPemegang().setFlag_dis(0);
			}else{
				editBac.getPemegang().setFlag_dis(1);
			}
...
//			refData.put("select_pendidikan", elionsManager.selectDropDown("eka.lst_education", "lsed_id", "lsed_name", "lsed_id", null));
		...
		/** Halaman 3 : Data Usulan Asuransi */
			...
			refData.put("listprodukutama", this.uwManager.select_produkutamamall());
			refData.put("listtipeproduk",this.uwManager.select_tipeprodukmall());
...
//			refData.put("select_autodebet",DroplistManager.getInstance().get("AUTODEBET.xml","ID",request));
			...
			Cmdeditbac editBac = (Cmdeditbac) cmd;
			editBac.getDatausulan().setKodeproduk(editBac.getDatausulan().getLsbs_id().toString());			
			String kode_produk=editBac.getDatausulan().getLsbs_id().toString();
			String number_produk=editBac.getDatausulan().getLsdbs_number().toString();
			editBac.getDatausulan().setPlan(kode_produk+"~X"+number_produk);
			if(editBac.getDatausulan().getLsbs_id() != null) {
				if(editBac.getDatausulan().getLsbs_id()==0){
					editBac.getDatausulan().setLsbs_id(null);
				}
			}
			if(editBac.getDatausulan().getLsbs_id() != null) {
				if(editBac.getDatausulan().getLsdbs_number()==null){
					editBac.getDatausulan().setLsdbs_number(null);
				}else{
			...
			refData.put("select_kombinasi",DroplistManager.getInstance().get("KOMBINASI_PREMI_MALL.XML","ID",request));
		...
		/** Halaman 4 : Detail Investasi */
			...
			refData.put("select_autodebet",DroplistManager.getInstance().get("AUTODEBET_MALL.xml","ID",request));
			...
			if(currentUser.getLde_id().equals("11") || currentUser.getLde_id().equals("29")){
			...
			if(!currentUser.getCab_bank().equals("") && !currentUser.getName().contains("SIMASPRIMA")) {
				...
				refData.put("select_hubungan",DroplistManager.getInstance().get("RELATION_MALL2.xml","",request));
			}else {
		...
		/** Halaman 5 : Detail Agen*/
			...
			if(currentUser.getLca_id().equals("58")){
				refData.put("select_regional", elionsManager.selectDropDown("eka.lst_region", "(lca_id||lwk_id||lsrg_id)", "lsrg_nama", "", "lsrg_nama", "lca_id in (\'58\')"));
			}else{
	...
	protected Object formBackingObject(HttpServletRequest request)
		...
		if(detiledit != null) { // SUMBER DATA : DARI GAGAL SUBMIT SEBELUMNYA
			...
			if(spaj!=null) {
				...
				if (products.stableLink(detiledit.getDatausulan().getLsbs_id().toString())) {
					...
					if (dt == null) {
						...
						inv.setDaftarinvestasi(this.uwManager.selectDetailInvestasiMall(null));
					...
					InvestasiUtama investasi  = (InvestasiUtama)this.uwManager.selectinvestasiutamamall(spaj);
					if (investasi == null) {
						InvestasiUtama inv = new InvestasiUtama();
			...
			/** INSERT SPAJ BARU */
				...
				InvestasiUtama inv = new InvestasiUtama();
				detiledit.getPemegang().setMcl_first(currentUser.getMall_nama_pp());
				detiledit.getPemegang().setEmail(currentUser.getMall_email());
				detiledit.getPemegang().setNo_hp(currentUser.getMall_hp());
				detiledit.getPemegang().setArea_code_rumah(currentUser.getMall_kd_area_telp());
				detiledit.getPemegang().setTelpon_rumah(currentUser.getMall_telp());
				detiledit.getPemegang().setMspe_date_birth(currentUser.getMall_tgl_lhr_pp());
				detiledit.getPemegang().setMspo_plan_provider(currentUser.getMall_mspo_plan_provider());
				if(currentUser.getData_usulan()!=null){
					detiledit.getDatausulan().setLsbs_id(currentUser.getData_usulan().getLsbs_id());
					detiledit.getDatausulan().setLsdbs_number(currentUser.getData_usulan().getLsdbs_number());
					detiledit.getDatausulan().setLscb_id(currentUser.getData_usulan().getLscb_id());
					detiledit.getDatausulan().setMspr_tsi(currentUser.getData_usulan().getMspr_tsi());
					detiledit.getDatausulan().setMspr_premium(currentUser.getData_usulan().getMspr_premium());
					detiledit.getDatausulan().setLku_id(currentUser.getData_usulan().getLku_id());
					detiledit.getDatausulan().setDaftaRider(currentUser.getData_usulan().getDaftaRider());
					if(currentUser.getData_usulan().getFlag_paket()==null)currentUser.getData_usulan().setFlag_paket(0);
					detiledit.getDatausulan().setFlag_paket(currentUser.getData_usulan().getFlag_paket());
				}
				...
				detiledit.getAgen().setMsag_id(currentUser.getMall_msag_id());
				detiledit.getAgen().setMcl_first(currentUser.getMall_nama_penutup());
...
//						harus punya hak akses untuk mengkopi data
						...
						if (akses.contains(region) || currentUser.getLca_id().equals("01")) {
							...
							detiledit.getTertanggung().setDaftarDth(uwManager.select_dth(kopiSPAJ));
							detiledit.setAgen(elionsManager.select_detilagen(kopiSPAJ));
						}
					}
				}
				
				if(!currentUser.getCab_bank().equals("") && (currentUser.getJn_bank().intValue() == 0 || currentUser.getJn_bank().intValue() == 1)) {
	...
	protected void onBind(HttpServletRequest request, Object cmd, BindException errors) throws Exception {
		...
		/** HALAMAN TERTANGGUNG */
			...
			List<Simas> c = new ArrayList<Simas>();
			List<Datarider> b = new ArrayList<Datarider>();
			List<PesertaPlus_mix> pp_mix = new ArrayList<PesertaPlus_mix>();
			List<PesertaPlus> pp = new ArrayList<PesertaPlus>();
			Cmdeditbac a = (Cmdeditbac) cmd;
			...
			if (jmlh_rider>0){
				for (int k = 1 ;k <= jmlh_rider; k++)	{
					...
					if("819,820,823,825,826,831,832,833".indexOf(kd_rd.toString())>-1){
...
//					set_pp_mix.setPlan_rider(bisnis);
			...
			}else if(jmlh_rider == 0 && "183,189,193,195,201,204".indexOf(a.getDatausulan().getLsbs_id().toString())>-1){
			...
			logger.info(a.getDatausulan().getDaftarplus_mix());
			...
			List<PesertaPlus_mix> xxxMix = new ArrayList<PesertaPlus_mix>();
			...
			if(jumlah_peserta > 0){
				for(int x = 1; x <= jumlah_peserta; x++){
					...
					if(jmlFlagJenis == null){
						jmlFlagJenis = 0;
					}
					if(lsbs_id_plus_int == null){
						lsbs_id_plus_int = 0;
					}
					if(nama == null){
						nama = "";
...
//					if(x>0 && flagJenis="0" && tanggallahir!=null){
					...
					set_pp.setLsbs_id_plus(Integer.parseInt(lsbs_id_plus));
						...
						List test = a.getDatausulan().getDaftaRider(); // *Banyaknya Rider yang diambil
						Integer jumlah_r = new Integer(test.size());
							...
							for (int k =0 ; k <jumlah_r.intValue();k++)
							{
								...
								if(x_jenis.compareTo(jmlFlagJenis) == 0){
									if(x_lsbsId.compareTo(lsbs_id_plus_int) == 0){
...
//				}
				List test = a.getDatausulan().getDaftaRider();
			...
			List<PesertaPlus> plus2 = a.getDatausulan().getDaftarplus();
			...
			jml_flagJP = a.getDatausulan().getFlag_compare();
				...
				List test2 = a.getDatausulan().getDaftaRider();
		...
		/** HALAMAN DATA USULAN */
			...
			List<RencanaPenarikan> c = new ArrayList<RencanaPenarikan>();
			...
			if (jmlh_dth>0){
				for (int k = 1 ;k <= jmlh_dth; k++){
					...
					rp.setJumlah(nf.parse(jumlah).doubleValue());
...
//			User currentUser = (User) request.getSession().getAttribute("currentUser");
			...
			detiledit.getDatausulan().setTipeproduk(13); //sinarmas sekuritas
//			detiledit.getAgen().setMsag_id("585858"); //kode penutup : SIMAS SEKURITAS
//			detiledit.getPemegang().setMspo_ao("585858");
...
////				params.put("success", "Berhasil Update Tanggal Terima Admin");
			...
			return new ModelAndView("bac/editsubmitmall",m);
