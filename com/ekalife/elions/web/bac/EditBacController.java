 ...
 */
public class EditBacController extends ParentWizardController {
...
//			}
			...
			if(editBac.getDatausulan().getLsbs_id() != null && editBac.getDatausulan().getLsbs_id() !=0) {
				if(editBac.getDatausulan().getLsdbs_number()!= null && editBac.getDatausulan().getLsdbs_number() !=0)
...
//			}
			...
			String flag_upload = ServletRequestUtils.getStringParameter(request, "flag_upload", "0");
...
//							    
//							    if(lsbs_id_rider!=null) datarider.get(i).setLsbs_id(lsbs_id_rider.intValue());
//							    if(lsdbs_no_rider!=null) datarider.get(i).setLsdbs_number(lsdbs_no_rider.intValue());
//							    if(persen_up_rider!=null){
//							    	datarider.get(i).setPersenUp(persen_up_rider.intValue());
//							    	datarider.get(i).setPersenUpx(persen_up_rider.intValue());
//							    }
...
//						}
					    ...
					    ArrayList< PesertaPlus_mix>peserta_plus=new ArrayList(this.bacManager.selectPesertaTemp(no_temp));
						    if (pemegangTemp==null){
								...
								detiledit.setPemegang(pemegangTemp);
								detiledit.getPemegang().setFlag_upload(Integer.parseInt(flag_upload));
								//detiledit.getPemegang().setId_proposal(no_temp);
							}
						//detiledit.getPemegang().setFlag_upload(Integer.parseInt(flag_upload));
						detiledit.setNo_temp(no_temp);
						if(ttgTemp!=null){
						...
						detiledit.getDatausulan().setDaftabenef(new ArrayList(bacManager.select_benef_temp(no_temp)));
						Agen agen = new Agen();
					    agen = uwManager.select_detilagen2(detiledit.getPemegang().getMsag_id());
						detiledit.setAddressbilling(addrBillTemp);
						if(pemegangTemp!=null){
						...
						Rekening_client rekTemp2= this.bacManager.select_rek_client_temp(no_temp);
						if(rekTemp2 != null) {
							detiledit.setRekening_client(rekTemp2);
							detiledit.getRekening_client().setMrc_no_ac_split(FormatString.splitWordToCharacter(detiledit.getRekening_client().getMrc_no_ac(),21));
						}
...
//				}
		...
		if(file.exists() && !refreshObject.equals("2") && flagAwal.equals("1")){
			...
			if(detiledit!=null){
				detiledit.setFileConfirm("Perhatian, Sebelumnya pernah dilakukan penginputan untuk nasabah " +detiledit.getPemegang().getMcl_first()+ ". Apakah Anda ingin meneruskan penginputannya? tekan OK apabila mau meneruskan, CANCEL apabila menginput data baru");
				detiledit.setFlag_special_case("0");
...
//					detiledit = null;
	...
	protected void onBind(HttpServletRequest request, Object cmd, BindException errors) throws Exception {
		...
		//	(Andhika) *Sumber Pendanaan pembelian asuransi tambahan
			...
			int juml_kyc = Integer.parseInt(request.getParameter("jmlDaftarKyc"));
		...
		//	(Andhika) *Sumber penghasilan tambahan
			...
			int juml_kyc2 = Integer.parseInt(request.getParameter("jmlDaftarKyc2"));
		...
		/** HALAMAN TERTANGGUNG */
			...
			if (jmlh_rider>0){
				for (int k = 1 ;k <= jmlh_rider; k++)	{
					...
					if("819,820,823,825,826,831,832,833,836,838".indexOf(kd_rd.toString())>-1){
