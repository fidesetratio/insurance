...
public class Editbacvalidatorspajnew implements Validator{
...
//				}
	...
	public void validatepp(Object cmd, Errors err)  throws ServletException,IOException{
        ...
        String flag_pemegang_polis=edit.getPemegang().getPemegang_polis();
        if (StringUtil.isEmpty(flag_pemegang_polis)){
        	
        	edit.getPemegang().setPemegang_polis("2");
        }
...
//		}
		...
		if ((nomor_seri==null||nomor_seri.equals("")) && !edit.getCurrentUser().getLca_id().equals("58") && !edit.getCurrentUser().getLus_id().equals("2661") /*&& "4,5".indexOf(edit.getPemegang().getMspo_flag_spaj().toString())<=-1*/){
...
//			nomor_seri="";
		...
		if(edit.getPemegang().getMspo_flag_spaj()>0){
			//String no_virtual_acc= edit.getTertanggung().getMste_no_vacc();
			if(nomor_seri.length()!=12)err.rejectValue("pemegang.mspo_no_blanko","","PEMEGANG: No Seri Untuk Form SPAJ baru harus berjumlah 12 digit angka");
//			if ((no_virtual_acc==null||no_virtual_acc.equals("")) && !edit.getCurrentUser().getLca_id().equals("58") && !edit.getCurrentUser().getLus_id().equals("2661") ){
//				ValidationUtils.rejectIfEmptyOrWhitespace(err, "tertanggung.mste_no_vacc", "", "PEMEGANG: Harap Lengkapi No Virtual Account Yang Tertera Pada Form Or Gadget");
//				
//			}else{
//				if(no_virtual_acc.length()!=16){
//					err.rejectValue("tertanggung.mste_no_vacc","","PEMEGANG: No Virtual Account Harus berjumlah 16 Digit");
//				}
//				String no_spaj_vacc=bacManager.selectno_virtual_sama(no_virtual_acc);
//				Integer no_spaj_vaccExist=bacManager.selectno_virtual_Exist(no_virtual_acc);
//				if(spaj.equalsIgnoreCase("")){
//					if(no_spaj_vacc!=null){	
//						if ((!no_spaj_vacc.equals("")) && !edit.getCurrentUser().getLca_id().equals("58") && !edit.getCurrentUser().getLus_id().equals("2661") ){
//							err.rejectValue("tertanggung.mste_no_vacc","","PEMEGANG: No Virtual Account : "+ no_virtual_acc + " Sudah Pernah Digunakan SPAJ : "+no_spaj_vacc);
//							
//						}
//					}
//				}	
//				if(no_spaj_vaccExist<=0){
//						err.rejectValue("tertanggung.mste_no_vacc","","PEMEGANG: No Virtual Account tidak terdaftar silahkan cek nomor yang terdapat pada form SPAJ");
//				}				
//			}	
...
//				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.mspo_no_blanko", "", "PEMEGANG: NO BLANKO YANG DICANTUMKAN SUDAH PERNAH DIGUNAKAN UNTUK PRODUK YANG SAMA");
		 ...
		 */
		if (flag_worksite != null && !edit.getCurrentUser().getLus_id().equals("2661")/*&&  "4,5".indexOf(edit.getPemegang().getMspo_flag_spaj().toString())<=-1*/){
...
//						edit.getDatausulan().setMste_flag_el(new Integer(1));
		...
		if(edit.getDatausulan().getJenis_pemegang_polis() == 1){
			...
			//validasi untuk inputan tambahan badan hukum
			ValidationUtils.rejectIfEmptyOrWhitespace(err, "personal.mpt_no_anggaran","","PIC: Silahkan masukkan nomor anggaran dasar");
			ValidationUtils.rejectIfEmptyOrWhitespace(err, "personal.mpt_tgl_anggaran", "","PIC: Silahkan masukkan tanggal anggaran dasar");
			ValidationUtils.rejectIfEmptyOrWhitespace(err, "personal.mpt_notaris","","PIC: Silahkan masukkan nama Notaris");
		}else{
			ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.mcl_first","","PEMEGANG: Silahkan masukkan nama Pemegang Polis");
			if(edit.getPemegang().getMkl_kerja().equals("0")){
				err.rejectValue("pemegang.mkl_kerja", "", "PEMEGANG: Silahkan memilih pekerjaan ");
			}
			if(!(edit.getPemegang().getMkl_kerja().equals("324") || edit.getPemegang().getMkl_kerja().equals("331"))){
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.mcl_company_name","","PEMEGANG: Silahkan masukkan nama perusahaan");
			}
			ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.mkl_kerja_ket","","PEMEGANG: Silahkan masukkan uraian tugas");
			ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.mcl_npwp","","PEMEGANG: Silahkan masukkan nomor npwp pemegang polis");
			
			if(edit.getPemegang().getMspo_jenis_terbit()==1){
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "addressbilling.e_mail","","PEMEGANG: Jika memilih pengiriman email harap masukkan alamat email");
			}
		}
		...
		edit.getPersonal().setMpt_no_anggaran(f_validasi.convert_karakter(edit.getPersonal().getMpt_no_anggaran()));
		edit.getPersonal().setMpt_notaris(f_validasi.convert_karakter(edit.getPersonal().getMpt_notaris()));
		edit.getPersonal().setMpt_tgl_anggaran(edit.getPersonal().getMpt_tgl_anggaran());
		
		edit.getPemegang().setMcl_first_alias(f_validasi.convert_karakter(edit.getPemegang().getMcl_first_alias()));
		edit.getPemegang().setMcl_green_card(edit.getPemegang().getMcl_green_card());
...
//			}
		...
		//					}
						...
						if(no_blanko.length()!=12 /*&&  "4,5".indexOf(edit.getPemegang().getMspo_flag_spaj().toString())<=-1*/){
							err.rejectValue("pemegang.mspo_no_blanko", "", "Untuk Jenis Form SPAJ BARU / SIO / GIO no blanko harus berjumlah 12 digit ");
						}
		...
		if (edit.getDatausulan().getJenis_pemegang_polis()==0 && !edit.getPemegang().getMspe_sts_mrt().equalsIgnoreCase("3")){	

			if (edit.getPemegang().getPemegang_polis().equals("0")){
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.nama_suami", "", "PEMEGANG: Silahkan masukkan nama suami");
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.tgl_suami", "", "PEMEGANG: Silahkan masukkan tanggal lahir suami");
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.pekerjaan_suami", "", "PEMEGANG: Silahkan masukkan pekerjaan Suami");
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.jabatan_suami", "", "PEMEGANG: Silahkan masukkan jabatan Suami");
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.perusahaan_suami", "", "PEMEGANG: Silahkan masukkan perusahaan tempat bekerja Suami");
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.bidang_suami", "", "PEMEGANG: Silahkan masukkan bidang usaha Suami");
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.npwp_suami", "", "PEMEGANG: Silahkan masukkan No. NPWP Suami");
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.penghasilan_suami", "", "PEMEGANG: Silahkan masukkan penghasilan Suami");
				
			}else if (edit.getPemegang().getPemegang_polis().equals("1")){
				
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.nama_ayah", "", "PEMEGANG: Silahkan masukkan nama Ayah");
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.tgl_ayah", "", "PEMEGANG: Silahkan masukkan tanggal lahir Ayah");
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.pekerjaan_ayah", "", "PEMEGANG: Silahkan masukkan pekerjaan Ayah");
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.jabatan_ayah", "", "PEMEGANG: Silahkan masukkan jabatan Ayah");
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.perusahaan_ayah", "", "PEMEGANG: Silahkan masukkan perusahan tempat bekerja Ayah");
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.bidang_ayah", "", "PEMEGANG: Silahkan masukkan bidang usaha Ayah");
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.npwp_ayah", "", "PEMEGANG: Silahkan masukkan No. NPWP Ayah");
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.penghasilan_ayah", "", "PEMEGANG: Silahkan masukkan penghasilan Ayah");
				
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.nama_ibu", "", "PEMEGANG: Silahkan masukkan nama Ibu");
				ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.tgl_ibu", "", "PEMEGANG: Silahkan masukkan tanggal lahir Ibu");
				
			}else if (edit.getPemegang().getPemegang_polis().equals("4")){
				err.rejectValue("pemegang.pemegang_polis","","PEMEGANG: Harap pilih calon pemegang polis");
			}
			
			//data pekerjaan
			edit.getPemegang().setMcl_company_name(f_validasi.convert_karakter(edit.getPemegang().getMcl_company_name()));
			edit.getPemegang().setMkl_kerja_ket(f_validasi.convert_karakter(edit.getPemegang().getMkl_kerja_ket()));
			//edit.getTertanggung().setMkl_kerja_ket(f_validasi.convert_karakter(edit.getPemegang().getMkl_kerja_ket()));
			
			
			//Data keluarga
			//data suami
			edit.getPemegang().setNama_suami(f_validasi.convert_karakter(edit.getPemegang().getNama_suami()));
			edit.getPemegang().setTgl_suami(edit.getPemegang().getTgl_suami());
			edit.getPemegang().setPekerjaan_suami(edit.getPemegang().getPekerjaan_suami());
			edit.getPemegang().setPerusahaan_suami(f_validasi.convert_karakter(edit.getPemegang().getPerusahaan_suami()));
			edit.getPemegang().setJabatan_suami(f_validasi.convert_karakter(edit.getPemegang().getJabatan_suami()));
			edit.getPemegang().setBidang_suami(edit.getPemegang().getBidang_suami());
			edit.getPemegang().setNpwp_suami(f_validasi.convert_karakter(edit.getPemegang().getNpwp_suami()));
			edit.getPemegang().setPenghasilan_suami(edit.getPemegang().getPenghasilan_suami());
			
			//data ayah
			edit.getPemegang().setNama_ayah(f_validasi.convert_karakter(edit.getPemegang().getNama_ayah()));
			edit.getPemegang().setTgl_ayah(edit.getPemegang().getTgl_ayah());
			edit.getPemegang().setPekerjaan_ayah(edit.getPemegang().getPekerjaan_ayah());
			edit.getPemegang().setJabatan_ayah(f_validasi.convert_karakter(edit.getPemegang().getJabatan_ayah()));
			edit.getPemegang().setPerusahaan_ayah(f_validasi.convert_karakter(edit.getPemegang().getPerusahaan_ayah()));
			edit.getPemegang().setBidang_ayah(edit.getPemegang().getBidang_ayah());
			edit.getPemegang().setNpwp_ayah(f_validasi.convert_karakter(edit.getPemegang().getNpwp_ayah()));
			edit.getPemegang().setPenghasilan_ayah(edit.getPemegang().getPenghasilan_ayah());
			
			//data ibu
			edit.getPemegang().setNama_ibu(f_validasi.convert_karakter(edit.getPemegang().getNama_ibu()));
			edit.getPemegang().setTgl_ibu(edit.getPemegang().getTgl_ibu());
			edit.getPemegang().setPekerjaan_ibu(edit.getPemegang().getPekerjaan_ibu());
			edit.getPemegang().setPerusahaan_ibu(f_validasi.convert_karakter(edit.getPemegang().getPerusahaan_ibu()));
			edit.getPemegang().setJabatan_ibu(f_validasi.convert_karakter(edit.getPemegang().getJabatan_ibu()));
			edit.getPemegang().setBidang_ibu(edit.getPemegang().getBidang_ibu());
			edit.getPemegang().setNpwp_ibu(f_validasi.convert_karakter(edit.getPemegang().getNpwp_ibu()));
			edit.getPemegang().setPenghasilan_ibu(edit.getPemegang().getPenghasilan_ibu());
			
		}
		
		
		//pekerjaan
		edit.getPemegang().setKerjaNote(bacManager.selectKeteranganKerja(edit.getPemegang().getMkl_kerja()));
		//gelar_pp
...
//			ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.lska_id", "", "PEMEGANG: Harap Pilih Kabupaten Pemegang Polis");
//			ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.lskc_id", "", "PEMEGANG: Harap Pilih Kecamatan Pemegang Polis");
//			ValidationUtils.rejectIfEmptyOrWhitespace(err, "pemegang.lskl_id", "", "PEMEGANG: Harap Pilih Kelurahan Pemegang Polis");
		}
...
//		edit.getPemegang().setLska_id(f_validasi.convert_karakter(edit.getPemegang().getLska_id()));
//		edit.getPemegang().setLskc_id(f_validasi.convert_karakter(edit.getPemegang().getLskc_id()));
//		edit.getPemegang().setLskl_id(f_validasi.convert_karakter(edit.getPemegang().getLskl_id()));
		edit.getPemegang().setAlamat_tpt_tinggal(f_validasi.convert_karakter(edit.getPemegang().getAlamat_tpt_tinggal()));
		edit.getPemegang().setKota_tpt_tinggal(f_validasi.convert_karakter(edit.getPemegang().getKota_tpt_tinggal()));
		edit.getPemegang().setKd_pos_tpt_tinggal(f_validasi.convert_karakter(edit.getPemegang().getKd_pos_tpt_tinggal()));
		edit.getPemegang().setMcl_npwp(f_validasi.convert_karakter(edit.getPemegang().getMcl_npwp()));
...
//		}
		...
		//alamat_tempat tinggal_pp	- ridhaal	
		String almt_tpttgl_pp=edit.getPemegang().getAlamat_tpt_tinggal();
		if (almt_tpttgl_pp==null)
		{
			almt_tpttgl_pp="";
		}
		if (almt_tpttgl_pp.trim().length()!=0)
		{
			edit.getPemegang().setAlamat_tpt_tinggal(f_validasi.convert_karakter(almt_tpttgl_pp));
		}
		...
		//kota_tempattgl_pp	- Ridhaal	
				String kota_tpttgl_pp=edit.getPemegang().getKota_tpt_tinggal();
				String hasil_kota_tpttgl_pp="";
				if (kota_tpttgl_pp==null)
				{
					kota_tpttgl_pp = "";
					edit.getPemegang().setKota_tpt_tinggal("");
					kota_tpttgl_pp=edit.getPemegang().getKota_tpt_tinggal();
				}
				
				if (almt_tpttgl_pp.trim().length()!=0)
				{
					hasil_kota_tpttgl_pp=pp.kota_tempattgl_pp(kota_tpttgl_pp);
					if(hasil_kota_tpttgl_pp.trim().length()!=0)
					{
						edit.getPemegang().setKota_tpt_tinggal(f_validasi.convert_karakter(kota_tpttgl_pp));
						if (jumlah_cancel.intValue()==0 && flag_gutri.intValue() ==0 && mste_flag_guthrie.intValue() == 0)
						{
							err.rejectValue("pemegang.kota_tpt_tinggal","","PEMEGANG:" +hasil_kota_tpttgl_pp);
						}
					}
				}
				edit.getPemegang().setKota_tpt_tinggal(f_validasi.convert_karakter(kota_tpttgl_pp));
		...
		//kode_pos_tpttgl_pp -- Ridhaal		
		String kd_pos_tpttgl_pp=edit.getPemegang().getKd_pos_tpt_tinggal();
		if (kd_pos_tpttgl_pp==null)
		{
			kd_pos_tpttgl_pp="";
		}
		String hasil_kd_pos_tpttgl_pp = pp.kode_pos_tempattgl_pp(kd_pos_tpttgl_pp);
		if (hasil_kd_pos_tpttgl_pp.trim().length()!=0)
		{
			edit.getPemegang().setKd_pos_tpt_tinggal(f_validasi.convert_karakter(kd_pos_tpttgl_pp));
			if (jumlah_cancel.intValue()==0 && flag_gutri.intValue() ==0 && mste_flag_guthrie.intValue() == 0)
			{
				err.rejectValue("pemegang.kd_pos_tpt_tinggal","","PEMEGANG:" +hasil_kd_pos_tpttgl_pp);
			}
		}
		edit.getPemegang().setKd_pos_tpt_tinggal(f_validasi.convert_karakter(kd_pos_tpttgl_pp));
		...
		//email pp
		String email_pp1=edit.getAddressbilling().getE_mail();
...
//		}
		...
		if (tagih.equalsIgnoreCase("2"))
		{
			...
			}else if (tagih.equalsIgnoreCase("4"))
			{
				almt_tgh=edit.getPemegang().getAlamat_tpt_tinggal();
...
//		*Sumber Premi Perorangan atau Badan Usaha
		...
		//set pendapatan & tujuan yang baru KYC Pemegang
		Boolean errPendapatanBulan = false;
		Boolean errTujuan = false;
		Boolean errPendapatanTahun = false;
		String concat_pendanaan="";
		String concat_tujuan="";
		Integer jenis_pp = edit.getDatausulan().getJenis_pemegang_polis();
		String [] pendapatanRutinBulan = {
				f_validasi.gantiKata(edit.getPemegang().getBulan_gaji()),
				f_validasi.gantiKata(edit.getPemegang().getBulan_penghasilan()),
				f_validasi.gantiKata(edit.getPemegang().getBulan_orangtua()),
				f_validasi.gantiKata(edit.getPemegang().getBulan_usaha()+";"+f_validasi.gantiKata(edit.getPemegang().getBulan_usaha_note())),
				f_validasi.gantiKata(edit.getPemegang().getBulan_laba()),
				f_validasi.gantiKata(edit.getPemegang().getBulan_lainnya()+";"+f_validasi.gantiKata(edit.getPemegang().getBulan_lainnya_note()))
		};
		String [] tujuanInvestasi = {
				f_validasi.gantiKata(edit.getPemegang().getTujuan_proteksi()),
				f_validasi.gantiKata(edit.getPemegang().getTujuan_pendidikan()),
				f_validasi.gantiKata(edit.getPemegang().getTujuan_investasi()),
				f_validasi.gantiKata(edit.getPemegang().getTujuan_tabungan()),
				f_validasi.gantiKata(edit.getPemegang().getTujuan_pensiun()),
				f_validasi.gantiKata(edit.getPemegang().getTujuan_lainnya()+";"+f_validasi.gantiKata(edit.getPemegang().getTujuan_lainnya_note()))
		};
		
		edit.getPemegang().setPendapatanBulan(pendapatanRutinBulan);
		edit.getPemegang().setTujuanInvestasi(tujuanInvestasi);
		
		//untuk gabungin pendapatan menjadi 1 di mkl_pendanaan
		//untuk validasi 
		...
		//untuk diset ke mkl_pendanaan
		for(int i=0;i<pendapatanRutinBulan.length;i++){
			if(pendapatanRutinBulan[i].contains("-")){
				
			}else{
				concat_pendanaan=concat_pendanaan.concat(pendapatanRutinBulan[i]+";");
			}
		}
		
		//validasi sumber pendapatan rutin 
//		if(!f_validasi.gantiKata(edit.getPemegang().getTahun_investasi()).equals("-") && f_validasi.gantiKata(edit.getPemegang().getTahun_investasi_note()).equals("-")){
//			err.rejectValue("pemegang.tahun_investasi","","PEMEGANG(Sumber Pendapatan rutin per tahun): Jika memilih hasil investasi,harap dijelaskan hasil investasinya" );
//		}else if(!f_validasi.gantiKata(edit.getPemegang().getBulan_lainnya()).equals("-") && f_validasi.gantiKata(edit.getPemegang().getBulan_lainnya_note()).equals("-")){
//			err.rejectValue("pemegang.tahun_lainnya","","PEMEGANG(Sumber Pendanaan): Jika memilih lainnya,harap dijelaskan" );
//		}
		...
		for(int i=0;i<tujuanInvestasi.length;i++){
			if(tujuanInvestasi[i].contains("-") ){
				...
				errTujuan = false;
				concat_tujuan=concat_tujuan.concat(tujuanInvestasi[i]+"; ");
				break;
			}
		}
		
		//untuk di set di mkl_tujuan
		for(int i=0;i<tujuanInvestasi.length;i++){
			if(tujuanInvestasi[i].contains("-") ){
		...
		if(jenis_pp == 1){ 
			...
			if(f_validasi.gantiKata(edit.getPemegang().getMkl_tujuan()).equals("LAIN - LAIN") && f_validasi.gantiKata(edit.getPemegang().getTujuana()).equals("-")){
				err.rejectValue("pemegang.mkl_tujuan","","PEMEGANG (Tujuan Membeli Asuransi) : Jika memilih lain-lain, harap dijelaskan tujuannya" );
			}
			if(f_validasi.gantiKata(edit.getPemegang().getMkl_pendanaan()).equals("LAINNYA") && f_validasi.gantiKata(edit.getPemegang().getDanaa()).equals("-")){
				err.rejectValue("pemegang.mkl_pendanaan","","PEMEGANG (Sumber Dana) : Jika memilih lainnya, harap dijelaskan sumbernya" );
			}
		}else{
			edit.getPemegang().setMkl_pendanaan(concat_pendanaan);
			edit.getPemegang().setMkl_tujuan(concat_tujuan);

			if(errPendapatanBulan){
				err.rejectValue("pemegang.bulan_gaji","","PEMEGANG (Sumber Pendanaan) : Harus memilih sumber pendapatan rutin perbulan" );
			}
			if(!f_validasi.gantiKata(edit.getPemegang().getBulan_lainnya()).equals("-") && f_validasi.gantiKata(edit.getPemegang().getBulan_lainnya_note()).equals("-")){
				err.rejectValue("pemegang.bulan_lainnya","","PEMEGANG (Sumber Pendanaan) : Jika memilih lainnya, harap dijelaskan" );
			}
			if(errTujuan){
				err.rejectValue("pemegang.tujuan_lainnya","","PEMEGANG (Tujuan Membeli Asuransi) : Harus memilih tujuan pengajuan asuransi" );
			}
		}
...
/*		String sumber_dana= edit.getPemegang().getMkl_pendanaan();
		...
		//sumber penghasilan
		//Sumber penghasilan dipindahkan dari tertanggung
...
//		}
		...
		if(edit.getDatausulan().getJenis_pemegang_polis() == 0){
			edit.getPemegang().setMkl_kerja_ket(f_validasi.convert_karakter(edit.getPemegang().getMkl_kerja_ket()));
			edit.getPemegang().setCompany_name(f_validasi.convert_karakter(edit.getPemegang().getCompany_name()));
		}
...
//			}
		...
		//black list Pemegang 
		ArrayList blacklist = Common.serializableList(this.uwManager.selectBlacklist(edit.getPemegang().getMcl_first(), "mcl_first", edit.getPemegang().getMspe_date_birth()));
		if (!blacklist.isEmpty()){
			edit.getPemegang().setBlacklist(edit.getPemegang().getMcl_first());
		}
...
//				edit.getTertanggung().setDanaa2(edit.getPemegang().getDanaa2());
		...
		/*		if(edit.getPemegang().getFlag_upload()==3){
				...
				edit.getTertanggung().setMcl_green_card(edit.getPemegang().getMcl_green_card());
				edit.getTertanggung().setMkl_kerja(edit.getPemegang().getMkl_kerja());
				edit.getTertanggung().setMcl_company_name(edit.getPemegang().getMcl_company_name());
				edit.getTertanggung().setMkl_kerja_ket(edit.getPemegang().getMkl_kerja_ket());
				edit.getTertanggung().setAlamat_tpt_tinggal(edit.getPemegang().getAlamat_tpt_tinggal());
				edit.getTertanggung().setKota_tpt_tinggal(edit.getPemegang().getKota_tpt_tinggal());
				edit.getTertanggung().setKd_pos_tpt_tinggal(edit.getPemegang().getKd_pos_tpt_tinggal());
				edit.getTertanggung().setMcl_npwp(edit.getPemegang().getMcl_npwp());
				...
				//set kyc tujuan & sumber pendanaan
				edit.getTertanggung().setBulan_gaji(edit.getPemegang().getBulan_gaji());
				edit.getTertanggung().setBulan_penghasilan(edit.getPemegang().getBulan_penghasilan());
				edit.getTertanggung().setBulan_orangtua(edit.getPemegang().getBulan_orangtua());
				edit.getTertanggung().setBulan_usaha(edit.getPemegang().getBulan_usaha());
				edit.getTertanggung().setBulan_lainnya(edit.getPemegang().getBulan_lainnya());
				edit.getTertanggung().setBulan_lainnya_note(edit.getPemegang().getBulan_lainnya_note());
				
				edit.getTertanggung().setTujuan_proteksi(edit.getPemegang().getTujuan_proteksi());
				edit.getTertanggung().setTujuan_investasi(edit.getPemegang().getTujuan_investasi());
				edit.getTertanggung().setTujuan_lainnya(edit.getPemegang().getTujuan_lainnya());
				edit.getTertanggung().setBlacklist(edit.getPemegang().getBlacklist());
		
			}else{
				//dijalankan kalau edit
				if (!spaj.equalsIgnoreCase(""))
				{
					String laintt = edit.getTertanggung().getBulan_lainnya_note();
					edit.setTertanggung((Tertanggung)this.elionsManager.selectttg(edit.getPemegang().getReg_spaj()));
					edit.getTertanggung().setBulan_lainnya_note(laintt);
					if (edit.getTertanggung().getBulan_lainnya_note()==null) laintt="";
					if (laintt.equals("")){
						edit.getTertanggung().setBulan_lainnya("");
					}else{
						edit.getTertanggung().setBulan_lainnya("LAINNYA");
					}
					 String mklId2 = "0";
						mklId2 = bacManager.selectIdLstPekerjaan(edit.getTertanggung().getMkl_kerja());
						
						edit.getTertanggung().setMkl_kerja(mklId2);
				}else if (spaj.equalsIgnoreCase("")){
					if(edit.getCurrentUser().getLca_id().equals("58")){
						edit.setTertanggung(new Tertanggung());
					}else{
						if(edit.getPemegang().getFlag_upload()!=null){
							if(edit.getTertanggung().getMkl_kerja()!=null && edit.getPemegang().getFlag_upload()!=3){
								String mklId2 = "0";
								mklId2 = bacManager.selectIdLstPekerjaan(edit.getTertanggung().getMkl_kerja().toUpperCase());
								edit.getTertanggung().setMkl_kerja(mklId2);
							}
						}else{
...
//							edit.getTertanggung().setDanaa2(edit.getPemegang().getDanaa2());
					...
					/*		if(edit.getPemegang().getFlag_upload()==3){
							edit.getTertanggung().setMste_no_vacc(no_virtual_acc);}*/
							edit.getTertanggung().setMkl_industri(edit.getPemegang().getMkl_industri());
							edit.getTertanggung().setMkl_kerja(edit.getTertanggung().getMkl_kerja());
							edit.getTertanggung().setMkl_pendanaan(edit.getPemegang().getMkl_pendanaan());
							edit.getTertanggung().setMkl_smbr_penghasilan(edit.getPemegang().getMkl_smbr_penghasilan());
							edit.getTertanggung().setMkl_penghasilan(edit.getPemegang().getMkl_penghasilan());
							 edit.getTertanggung().setMkl_dana_from(edit.getPemegang().getMkl_dana_from());
...
//---------------------------tertanggung--------------------------------	
	...
	try {
		...
		edit.getTertanggung().setMcl_first(f_validasi.convert_karakter(nama_ttg));
		edit.getTertanggung().setMcl_first_alias(f_validasi.convert_karakter(edit.getTertanggung().getMcl_first_alias()));
		edit.getTertanggung().setMcl_green_card(edit.getTertanggung().getMcl_green_card());
		edit.getTertanggung().setMcl_company_name(f_validasi.convert_karakter(edit.getTertanggung().getMcl_company_name()));
		edit.getTertanggung().setMkl_kerja_ket(f_validasi.convert_karakter(edit.getTertanggung().getMkl_kerja_ket()));
		edit.getTertanggung().setMkl_kerja(edit.getTertanggung().getMkl_kerja());
...
//		String lska_id = edit.getTertanggung().getLska_id();
//		String lskc_id = edit.getTertanggung().getLskc_id();
//		String lskl_id = edit.getTertanggung().getLskl_id();
//		lska_id = (lska_id == null ? "": lska_id);
//		lskc_id = (lskc_id == null ? "": lskc_id);
//		lskl_id = (lskl_id == null ? "": lskl_id);
		if (almt_rmh_ttg==null)
		{
			almt_rmh_ttg="";
		}
		String hasil_almt_rmh_ttg = a.alamat_rumah_ttg(almt_rmh_ttg);
		/*String hasil_lska_id_ttg = a.kabupaten_ttg(lska_id);
		String hasil_lskc_id_ttg = a.kecamatan_ttg(lskc_id);
		String hasil_lskl_id_ttg = a.kelurahan_ttg(lskl_id);*/
		...
		/*if (hasil_lska_id_ttg.trim().length()!=0)
		{
			edit.getTertanggung().setLska_id(f_validasi.convert_karakter(hasil_lska_id_ttg));
			err.rejectValue("tertanggung.lska_id","","HALAMAN TERTANGGUNG :" +hasil_lska_id_ttg);
		}
		if (hasil_lskc_id_ttg.trim().length()!=0)
		{
			edit.getTertanggung().setLskc_id(f_validasi.convert_karakter(hasil_lskc_id_ttg));
			err.rejectValue("tertanggung.lskc_id","","HALAMAN TERTANGGUNG :" +hasil_lskc_id_ttg);
		}
		if (hasil_lskl_id_ttg.trim().length()!=0)
		{
			edit.getTertanggung().setLskl_id(f_validasi.convert_karakter(hasil_lskl_id_ttg));
			err.rejectValue("tertanggung.lskl_id","","HALAMAN TERTANGGUNG :" +hasil_lskl_id_ttg);
		}*/
		edit.getTertanggung().setAlamat_rumah(f_validasi.convert_karakter(almt_rmh_ttg));
//		edit.getTertanggung().setLska_id(f_validasi.convert_karakter(lska_id));
//		edit.getTertanggung().setLskc_id(f_validasi.convert_karakter(lskc_id));
//		edit.getTertanggung().setLskl_id(f_validasi.convert_karakter(lskl_id));
		
		edit.getTertanggung().setAlamat_tpt_tinggal(f_validasi.convert_karakter(edit.getTertanggung().getAlamat_tpt_tinggal()));
		edit.getTertanggung().setKota_tpt_tinggal(f_validasi.convert_karakter(edit.getTertanggung().getKota_tpt_tinggal()));
		edit.getTertanggung().setKd_pos_tpt_tinggal(f_validasi.convert_karakter(edit.getTertanggung().getKd_pos_tpt_tinggal()));
		...
		//black list Tertanggung 
		ArrayList blacklist = Common.serializableList(this.uwManager.selectBlacklist(edit.getTertanggung().getMcl_first(), "mcl_first", edit.getTertanggung().getMspe_date_birth()));
		if (!blacklist.isEmpty()){
			edit.getTertanggung().setBlacklist(edit.getTertanggung().getMcl_first());
		}
		
		//tujuan membeli asuransi
		//di form baru sudah tidak ada
	/*	String tujuan_asr_ttg= edit.getTertanggung().getMkl_tujuan();
		...
		//Set KYC tertanggung Untuk Tujuan & Pendanaan Tertanggung
		Boolean errPendapatanBulan = false;
		Boolean errTujuan = false;
		Boolean errPendapatanTahun = false;
		String concat_pendanaan="";
		String concat_tujuan="";
		if (edit.getTertanggung().getBulan_lainnya_note()==null)
				edit.getTertanggung().setBulan_lainnya_note("");
		
		if (edit.getTertanggung().getBulan_lainnya_note()==""){
			edit.getTertanggung().setBulan_lainnya("");
		}
		
		String [] pendapatanRutinBulan = {
				f_validasi.gantiKata(edit.getTertanggung().getBulan_gaji()),
				f_validasi.gantiKata(edit.getTertanggung().getBulan_penghasilan()),
				f_validasi.gantiKata(edit.getTertanggung().getBulan_orangtua()),
				f_validasi.gantiKata(edit.getTertanggung().getBulan_usaha()+";"+f_validasi.gantiKata(edit.getTertanggung().getBulan_usaha_note())),
				f_validasi.gantiKata(edit.getTertanggung().getBulan_laba()),
				f_validasi.gantiKata(edit.getTertanggung().getBulan_lainnya()+";"+f_validasi.gantiKata(edit.getTertanggung().getBulan_lainnya_note()))
		};
		String [] tujuanInvestasi = {
				f_validasi.gantiKata(edit.getTertanggung().getTujuan_proteksi()),
				f_validasi.gantiKata(edit.getTertanggung().getTujuan_pendidikan()),
				f_validasi.gantiKata(edit.getTertanggung().getTujuan_investasi()),
				f_validasi.gantiKata(edit.getTertanggung().getTujuan_tabungan()),
				f_validasi.gantiKata(edit.getTertanggung().getTujuan_pensiun()),
				f_validasi.gantiKata(edit.getTertanggung().getTujuan_lainnya()+";"+f_validasi.gantiKata(edit.getTertanggung().getTujuan_lainnya_note()))
		};
		
		edit.getTertanggung().setPendapatanBulan(pendapatanRutinBulan);
		edit.getTertanggung().setTujuanInvestasi(tujuanInvestasi);
		...
		edit.getTertanggung().setMkl_pendanaan(concat_pendanaan);			
		if(errPendapatanBulan){
			err.rejectValue("tertanggung.bulan_gaji","","TERTANGGUNG : Harus memilih sumber pendapatan rutin perbulan" );
		}else if(!f_validasi.gantiKata(edit.getTertanggung().getBulan_lainnya()).equals("-") && f_validasi.gantiKata(edit.getTertanggung().getBulan_lainnya_note()).equals("-")){
			err.rejectValue("tertanggung.bulan_lainnya","","TERTANGGUNG : Jika memilih lainnya, harap dijelaskan" );
		}
		
		//validasi sumber pendapatan rutin 
		if(!f_validasi.gantiKata(edit.getTertanggung().getTahun_investasi()).equals("-") && f_validasi.gantiKata(edit.getTertanggung().getTahun_investasi_note()).equals("-")){
			err.rejectValue("tertanggung.tahun_investasi","","TERTANGGUNG(Sumber Pendapatan rutin per tahun): Jika memilih hasil investasi,harap dijelaskan hasil investasinya" );
		}else if(!f_validasi.gantiKata(edit.getTertanggung().getBulan_lainnya()).equals("-") && f_validasi.gantiKata(edit.getTertanggung().getBulan_lainnya_note()).equals("-")){
			err.rejectValue("tertanggung.tahun_lainnya","","TERTANGGUNG(Sumber Pendanaan): Jika memilih lainnya,harap dijelaskan" );
		}
		...
		}edit.getTertanggung().setMkl_tujuan(concat_tujuan);
		if(errTujuan){
			err.rejectValue("tertanggung.tujuan_lainnya","","TERTANGGUNG: Harus memilih tujuan pengajuan asuransi" );
		}
...
*/
		//sumber penghasilan
		//sumber penghasilan sudah tidak ada d form baru
...
//					err.rejectValue("datausulan.mste_flag_cc", "", "HALAMAN DATA USULAN: Metode Bayar untuk Progressive Save hanya bisa Transfer/AutoDebet Rekening atau Kartu Kredit");
			...
			if ((edit.getDatausulan().getLsbs_id()==142 && edit.getDatausulan().getLsdbs_number()==2) && edit.getPowersave().getMps_roll_over()!=null){
				if(edit.getPowersave().getMps_roll_over()==1){
					//Edit Roll Over ALL Simas Prima dibuka kembali (helpdesk 125831 - SK No.005/AJSMSIG-SK/IV/2018) - Chandra
					/*Date mar31 = defaultDateFormat.parse("31/03/2016");
					if (edit.getDatausulan().getMste_beg_date().after(mar31)){
						err.rejectValue("powersave.mps_roll_over", "", "HALAMAN INVESTASI : Khusus Simas Prima (142-002) , Per efektif 01 APRIL 2016 , tidak bisa memilih jenis RO ALL");
					}*/
				}
			}
...
//			cek top up
			...
			if (((jns_top_up_berkala.intValue())==1 || (jns_top_up_tunggal.intValue())==2) && (kode_flag.intValue()==3) && produk.ii_bisnis_id!=190)
			{
			...
			if (hasil_jns_top_up.trim().length()==0)
			{
				...
				//SMILE LINK PRO 100 (Konven 217 dan Syariah 218) tidak melakukan pengecekan untuk pengabungan Top Up tunggal dan berkala.
				if (jns_top_up_tunggal.intValue() >0 && jns_top_up_berkala.intValue() >0 && (produk.ii_bisnis_id!=217 || produk.ii_bisnis_id!=218)) {
					...
					//pengecekan top up berkala untuk produk SMILE LINK PRO 100 (Konven dan Syariah)).
					if (jns_top_up_berkala.intValue() >0 && (produk.ii_bisnis_id==217 || produk.ii_bisnis_id==218)) {
			...
			//MANTA
			if(produk.ii_bisnis_id==190 && produk.ii_bisnis_no==9 && (jns_top_up_berkala.intValue()==1 || topup_berkala.doubleValue()>0)) {
				err.rejectValue("investasiutama.daftartopup.pil_berkala","","HALAMAN INVESTASI : Produk SMiLe LINK ULTIMATE tidak dapat mengambil Top up Berkala!");
			}
			...
			if (kode_flag.intValue()==3 && total_premium1.doubleValue()<50000000 && produk.ii_bisnis_id!=190)
			{
			...
			if ( kode_flag.intValue() != 11 && kode_flag.intValue() != 15 && kode_flag.intValue() != 16)
			{
				...
				if((produk.ii_bisnis_id == 220 && produk.ii_bisnis_no == 4) || //Chandra - biaya 5% untuk B Smile Protection
				   (produk.ii_bisnis_id == 224 && produk.ii_bisnis_no == 3) || //Chandra - biaya 5% untuk B Smile Protection Syariah, helpdesk [133899]
				   (produk.ii_bisnis_id == 134 && produk.ii_bisnis_no == 13)){ //helpdesk [142003] produk baru Smart Platinum Link RPUL BEL (134-13)
					li_pct_biaya = 5.0;
				}
...
//				(Deddy) validasi baru untuk stable save BII
				...
				if (jns_top_up0 == null || jns_top_up0.equalsIgnoreCase(""))
				{
					...
					if((lsbs_id==142 && edit.getDatausulan().getLsdbs_number()==11) && mgi!=36){
						err.rejectValue("powersave.mps_jangka_inv","", "HALAMAN INVESTASI :Untuk Produk SMART SAVE, Hanya bisa mengambil MTI/MGI 36 bulan");
					}
...
//			}
			...
			negara = new Integer[(jmlh_penerima.intValue())+1];
			instruksi = new String[(jmlh_penerima.intValue())+1];
			...
			for (int k=0 ; k < (jmlh_penerima.intValue()) ; k++)
			{
				...
				negara[k]=bf1.getLsne_id();
				instruksi[k]=bf1.getMsaw_ket();
				...
				//relasi selain insureable, harus mengisi instruksi khusus. //Kecuali Produk ERBE tidak perlu ada intruksi khusus
				if ("2,4,5,7,10".indexOf(relation_m[k].toString())<0 && !(produk.ii_bisnis_id==217 && produk.ii_bisnis_no==2)){
					if(instruksi[k].trim().length()==0){
					err.rejectValue("datausulan.daftabenef["+k+"].msaw_ket","","HALAMAN INVESTASI : Hubungan Ahli Waris Ada yang Tidak Insurable, Mohon diinput Instruksi Khusus" );}
				}
			...
			if("54,131,132,148,183,189,193,195,197,201,204".indexOf(edit.getDatausulan().getLsbs_id().toString())<=-1){
...
//				edit.getDatausulan().setMste_flag_el(new Integer(1));
		...
		//Validasi Untuk EDIT BEGDATE JIKA SUDAH ADA BILLING SUCCESIVE(RYAN)
		Integer countBillingSucc = bacManager.selectCountMstBillingSucc(edit.getPemegang().getReg_spaj());
		Insured insured =bacManager.selectMstInsuredAll(edit.getPemegang().getReg_spaj());
		if(countBillingSucc>0){
			if(!edit.getDatausulan().getMste_beg_date().equals(insured.getMste_beg_date())){
			err.reject("","HALAMAN INVESTASI : Perubahan Begdate Tidak Bisa Dilakukan, Karena Sudah Ada Billing SUCCESIVE (LANJUTAN) ");}
		}
	}
	...
	public void validateagen(Object cmd, Errors err) 
	{
		...
		if (flag_karyawan.intValue() == 1 || flag_karyawan.intValue() == 4)
		{
			...
			String nik=edit.getPemegang().getNik();
...
//		hasil_agen_leader=d_agen.kode_agen_leader(mspo_leader);
		...
		if(kode_penutup!=""){
			try{
				if(uwManager.selectAgentActiveOrNot(kode_penutup)==0){
					...
					if((Integer)mapAgent.get("FLAG_AGEN_SYARIAH") != 1 && bacManager.selectLineBusLstBisnis(edit.getDatausulan().getLsbs_id().toString())==3 ){
						err.rejectValue("agen.msag_id","","HALAMAN DETIL AGEN : Untuk produk syariah harus dari agen Penutup Cabang/Wakil Syariah / Bersetifikat Syariah" );
					}
				}
			}catch(Exception ex){
				//agen tidak terdaftar disistem sudah ditambah dibawah
			}
		...
		if((edit.getDatausulan().getLsbs_id().intValue()==190 && edit.getDatausulan().getLsdbs_number().intValue()==9) ||
				(edit.getDatausulan().getLsbs_id().intValue()==200 && edit.getDatausulan().getLsdbs_number().intValue()==7) ){
			if(!(edit.getAgen().getKode_regional().equals("37M105") || edit.getAgen().getKode_regional().equals("37M106") || edit.getAgen().getKode_regional().equals("37M107"))){
				err.rejectValue("agen.msag_id","","HALAMAN DETIL AGEN : Produk SMiLe LINK ULTIMATE hanya untuk Agen Penutup yang terdaftar di Cabang SIAP2U Agency / SIAP2U_2 / SIAP2U_3 .");
			}
			if(edit.getPemegang().getMspo_id_sponsor().equals("")){
				err.rejectValue("pemegang.mspo_id_sponsor","","HALAMAN DETIL AGEN : ID Sponsor harus diisi.");
			}
			if(edit.getPemegang().getMspo_id_place().equals("")){
				err.rejectValue("pemegang.mspo_id_place","","HALAMAN DETIL AGEN : ID Placement harus diisi.");
			}
		}
		...
		if(hasil_agen.trim().length()==0 )
		{
			...
			{		
				...
				edit.getAgen().setMspe_email((String)data.get("MSPE_EMAIL"));
				...
				if(edit.getCurrentUser().getCab_bank().equals("")  ) {
					if((!edit.getCurrentUser().getLca_id().equals("58") && edit.getCurrentUser().getJn_bank().intValue() != 2 && edit.getCurrentUser().getJn_bank().intValue() != 3)){
						if (hasil_agen.trim().length()==0)
						{
							if (jumlah_cancel.intValue() == 0 && flag_gutri.intValue() ==0 && mste_flag_guthrie.intValue() == 0)
							{
								if((!cbg.equalsIgnoreCase("09")) && (!lcaid.equalsIgnoreCase("73"))){
								...
								//Validasi Produk ERBE PACKAGE harus di ambil oleh Agent ERBE PACKAGE
								//AGENT ERBE PACKAGE tidak ada validasi pengecekan lisensi
								if ( ((!lcaid.equalsIgnoreCase("73")) || (lcaid.equalsIgnoreCase("73") && (!edit.getAgen().getLwk_id().equals("01")))) 
										   && edit.getDatausulan().getLsbs_id().intValue()==217 && edit.getDatausulan().getLsdbs_number().intValue()==2){
										
										err.rejectValue("agen.msag_id","","HALAMAN DETIL AGEN : Produk ERBE Package harus di ambil oleh Agent ERBE Package juga !");
								}
							}
						}
					}
	...
	//							}
								...
								if (produk1.flag_as==1)
								{
									...
									if((edit.getDatausulan().getLsbs_id().intValue()==118 && edit.getDatausulan().getLsdbs_number().intValue()==3)  ||(edit.getDatausulan().getLsbs_id().intValue()==118 && edit.getDatausulan().getLsdbs_number().intValue()==4))
									{
										allowed = false;
									};
...
//							edit.getDatausulan().setMste_flag_el(new Integer(1));
					...
					List<Map> agent_data =  bacManager.selectDaftarAgentBlacklist();
					if(agent_data.size() > 0){
						Map detail_data = (Map)agent_data.get(0);
						if(detail_data.get("AGENT_CODE").toString().indexOf(edit.getAgen().getMsag_id()) > -1){
							String error_desc = String.format("HALAMAN DETIL AGEN : kode agent [%s] ini diblok submission new business. mohon hubungi distribution support.", edit.getAgen().getMsag_id());
							err.rejectValue("agen.msag_id", "", error_desc);
						}
					}
				}
