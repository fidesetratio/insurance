...
//			
		...
		public String generateAllRds(ServletContext context, HttpServletRequest request, HttpServletResponse response, ElionsManager elionsManager, UwManager uwManager, BacManager bacManager, Properties props, Products products) throws Exception {
			int printType = 32; //generate_all
			String spaj = request.getParameter("spaj");
			User currentUser = (User) request.getSession().getAttribute("currentUser");
			//currentUser.setLus_id("1");
			Pemegang pmg =elionsManager.selectpp(spaj);
			boolean isValidated=true;
			boolean isBancass = ServletRequestUtils.getBooleanParameter(request, "isbancass", false);
			boolean isViewer = ServletRequestUtils.getBooleanParameter(request, "isviewer", false);
...
//			Integer flag = uwManager.flag_upload_scan(spaj);			
			...
			format = ServletRequestUtils.getStringParameter(request, "format", "");
				...
				//Cek File Proposal (Product Unit Link)
				List errors = new ArrayList();
				String cabang = elionsManager.selectCabangFromSpaj(spaj);
				String path ="";
				String pathTemp = "";
				...
				pathTemp = props.getProperty("pdf.dir")+"\\"+cabang+"\\"+spaj;				
				File destDir = new File(path);
				String lsbs = bacManager.getUwDao().selectBusinessId(spaj);
...
//					return new ModelAndView("uw/printpolis/printpolis_err", "error", errors);
			...
			try{	
				//20190208 Mark Valentino - Proses insert mspo_date_print dahulu, baru generate files
				elionsManager.updatePolicyAndInsertPositionSpaj(spaj, "mspo_date_print", currentUser.getLus_id(), 6, 1, "GENERATE OUTSOURCE (E-LIONS)", true, currentUser);

				PrintPolisAllPelengkap ppap = new PrintPolisAllPelengkap(context);
				Integer mspo_provider=uwManager.selectGetMspoProvider(spaj);
				ppap.generateFileRDS(request, response, mspo_provider, 2, elionsManager, uwManager, bacManager, props, products);
				
				List listDetBisnis = elionsManager.selectDetailBisnis(spaj);
				String detbisnis = (String) ((Map) listDetBisnis.get(0)).get("DETBISNIS");
...
//				listExclude = ((lsbs.equals("142")) && ("002,004,006,007".indexOf(Integer.parseInt(detbisnis)) > 0))
