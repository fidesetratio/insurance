...
public class InputPasBpDbdController extends ParentFormController{
	...
	protected void onBindAndValidate(HttpServletRequest request, Object cmd, BindException err) throws Exception {
		...
		if(!"".equals(pas.getMsag_id_pp())){
			agen = uwManager.select_detilagen_bp(pas.getMsag_id_pp());			
			if(agen.getMsag_id() != null){
				...
				err.reject("","Agen : Kode Agen PAS-BP tidak berlaku");
			}
		}
		...
		if(!"".equals(pas.getMsag_id())){
			...
			if(agen.getMsag_id() != null){
				...
				err.reject("","Agen : Kode Agen DBD-Agency tidak berlaku");
			}
	...
	if(!"".equals(pas.getMsag_id_pp()) && !"".equals(pas.getMsag_id())){		
		Map m =null;
		m = new HashMap();
		Map data = (HashMap) ajaxManager.validasiAgenPasBPDbd(pas.getMsag_id_pp());
		String namaAgenPASBP="";
		String noktpAgenPASBP="";
		Date tgllahirAgenPASBP=null;
		
		namaAgenPASBP = data.get("nama_penutup").toString();
		noktpAgenPASBP = data.get("noktp").toString();
		tgllahirAgenPASBP = (Date) data.get("tgllahir");
		
		m=ajaxManager.validasiAgenPasBPDbd(pas.getMsag_id());
		String namaAgenDBDAgency="";
		String noktpAgenDBDAgency="";
		Date tgllahirAgenDBDAgency=null;
		
		namaAgenDBDAgency = m.get("nama_penutup").toString();
		noktpAgenDBDAgency = m.get("noktp").toString();
		tgllahirAgenDBDAgency = (Date) m.get("tgllahir");
		
		if(!namaAgenPASBP.equalsIgnoreCase(namaAgenDBDAgency)){
			err.reject("","Agen : Nama Agen PAS-BP dan DBD-Agency tidak sama!");
		}
		if(!noktpAgenPASBP.equalsIgnoreCase(noktpAgenDBDAgency)){
			err.reject("","Agen : No.KTP Agen PAS-BP dan DBD-Agency tidak sama!");
		}
		if(!tgllahirAgenPASBP.equals(tgllahirAgenDBDAgency)){
			err.reject("","Agen : Tgl.Lahir Agen PAS-BP dan DBD-Agency tidak sama!");
		}
	}
//		if(!"".equals(agen_hsl)){
...
//		}
		...
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "msp_identity_no_tt", "","PAS : No. Identitas Tertanggung harus diisi");
		if(pas.getLsag_id() == 6){
		...
		if(pas.getLsre_id() == 1){
			if(!(umurPp>=17 && umurPp<=85)){ //batas umur 17 s/d 85 tahun
				err.reject("","PAS/DBD : Umur Pemegang/Tertanggung minimal 17 tahun & maksimal 85 tahun");
			}			
		}else
		{
			if(!(umurPp>=17 && umurPp<=85)){ //batas umur 17 s/d 85 tahun
				err.reject("","PAS/DBD : Umur Pemegang minimal 17 tahun & maksimal 85 tahun");
			}			
...
//			//CheckSum checkSum = new CheckSum();
//			lsdbs_number_no_kartu = uwManager.selectCekNoKartu(pas.getNo_kartu(), 0);
...
//		}
		...
		//AGEN
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "msag_id_pp", "","AGEN : Kode Agen PAS-BP harus diisi");
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "msag_id", "","AGEN : Kode Agen DBD-Agency harus diisi");
		
		String msag_id_pp = pas.getMsag_id_pp();
		String msp_pas_nama_pp = pas.getMsp_pas_nama_pp();
		String msp_full_name = pas.getMsp_full_name();
		Date msp_pas_beg_date = pas.getMsp_pas_beg_date();
		List existedInput = uwManager.selectValidasiInputPasBp(msag_id_pp,msp_pas_nama_pp,msp_full_name,msp_pas_beg_date);
		if(existedInput != null && existedInput.size() != 0){
			err.reject("","PAS : Kode Agen ini sudah pernah di input");
		}
...
//    }
	...
	protected ModelAndView onSubmit( HttpServletRequest request, HttpServletResponse response, Object cmd, BindException errors ) throws Exception
    {
		...
		//Proses Copy Data PAS ke DBD Utk Salinan Sebelum Insert
		pas.setLscb_id(3);		
		pas.setLsre_id(1);
		Integer lsreid = Integer.valueOf(request.getParameter("lsre_id"));
		dbd.setLsre_id(lsreid);
		
		if(dbd.getLsre_id() == 1){
			dbd.setMsp_full_name(pas.getMsp_pas_nama_pp());
			dbd.setMsp_pas_tmp_lhr_tt(pas.getMsp_pas_tmp_lhr_pp());
			dbd.setMsp_date_of_birth(pas.getMsp_pas_dob_pp());
			dbd.setMsp_identity_no_tt(pas.getMsp_identity_no());
			dbd.setMsp_sex_tt(pas.getMsp_sex_pp());
		}
		else{
			dbd.setMsp_full_name(pas.getMsp_full_name());
			dbd.setMsp_pas_tmp_lhr_tt(pas.getMsp_pas_tmp_lhr_tt());
			dbd.setMsp_date_of_birth(pas.getMsp_date_of_birth());
			dbd.setMsp_identity_no_tt(pas.getMsp_identity_no_tt());
			dbd.setMsp_sex_tt(pas.getMsp_sex_tt());			
		}
		...
		Integer msp_flag_cc_dbd = Integer.valueOf(request.getParameter("msp_flag_cc"));
		dbd.setMsp_flag_cc(msp_flag_cc_dbd);
		
		dbd.setProduk(2);//DEMAM BERDARAH
		dbd.setPremi(pas.getMsp_premi());
		dbd.setMsp_premi(pas.getMsp_premi());
		String mspup = request.getParameter("msp_up");
		dbd.setMsp_up(mspup);
		dbd.setJenis_pas("DBD AGENCY");
		dbd.setMsag_id(pas.getMsag_id());
		dbd.setMsag_id_pp(null);
		...
		pas.setMsag_id(pas.getMsag_id_pp());
		
		Integer pribadi_dbd = 0;		
		String pribadidbd = request.getParameter("pribadi_dbd");		
		if(pribadidbd != null){
			pribadi_dbd = Integer.valueOf(pribadidbd);	
		}
		dbd.setPribadi(pribadi_dbd);
		
		String kode_ao_dbd = request.getParameter("kode_ao_dbd");
		dbd.setKode_ao(kode_ao_dbd);
		//End Copy
		...
		if("insert".equals(request.getParameter("kata"))){
			...
			if(pas.getStatus() == 1){
				...
				String msp_fire_id_pas = pas.getMsp_fire_id();
				//UPDATE PAS sesuaikan dengan DBD untuk Insert DBD
				pas.setMsp_fire_id(null);
				pas.setMsp_id(null);
				pas.setLsre_id(dbd.getLsre_id());								
				pas.setMsp_full_name(dbd.getMsp_full_name());
				pas.setMsp_pas_tmp_lhr_tt(dbd.getMsp_pas_tmp_lhr_tt());
				pas.setMsp_date_of_birth(dbd.getMsp_date_of_birth());
				pas.setMsp_identity_no_tt(dbd.getMsp_identity_no_tt());
				pas.setMsp_sex_tt(dbd.getMsp_sex_tt());
										
				pas.setProduk(dbd.getProduk());//DEMAM BERDARAH
				pas.setPremi(dbd.getMsp_premi());
				pas.setMsp_premi(dbd.getMsp_premi());
				pas.setMsp_up(dbd.getMsp_up());				
				pas.setJenis_pas("DBD AGENCY");
				pas.setMsag_id(dbd.getMsag_id());
				pas.setMsag_id_pp(dbd.getMsag_id_pp());
				pas.setPribadi(dbd.getPribadi());
				pas.setKode_ao(dbd.getKode_ao());
				
				pas.setMsp_company_name(null);			
				pas.setMsp_company_usaha(null);
				pas.setMsp_company_jabatan(null);
				pas.setMsp_company_address(null);
				pas.setMsp_company_postal_code(null);
				pas.setMsp_company_area_code(null);
				pas.setMsp_company_phone_number(null);
				pas.setMsp_exist_bp(null);
				pas.setMsp_exist_bp_name(null);
				//END
				pas = uwManager.insertPas(pas, user);
				
				Integer msp_id = uwManager.selectGetPasIdFromFireId(pas.getMsp_fire_id());
				request.setAttribute("msp_id",msp_id);				
				request.setAttribute("successMessage","insert sukses. Fire Id PAS-BP: "+msp_fire_id_pas+" Fire Id DBD-Agency: "+pas.getMsp_fire_id());
			}			
//			pas.setLus_id(lus_id);
//			pas.setLus_login_name(user.getLus_full_name());
...
//		schedulerPas();
		...
		return new ModelAndView(
        "uw/input_pasbp_dbd").addObject("cmd",pas);
    }
	...
	protected Object formBackingObject(HttpServletRequest request)
		...
		String msag_id_cari = request.getParameter("msag_id_cari");
...
//			else{
			...
			if( agenPas.get(0).get("MSAG_ID") != null){
				//  Adrian (PASBP + DBD Agency)
					pas.setMsag_id(msag_id_cari);
