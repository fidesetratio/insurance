...
public class InputPasPartnerController extends ParentFormController{
	...
	protected void onBindAndValidate(HttpServletRequest request, Object cmd, BindException err) throws Exception {
		...
		if(!"".equals(pas.getMsag_id())){					
			 ...
			 if(pas.getJenis_bp() == 0){
				 agen = uwManager.select_detilagen_bp(pas.getMsag_id());
			 }else 
			 if(pas.getJenis_bp() == 1){
			    agen = uwManager.select_detilagen_bpd(pas.getMsag_id());
			 }
...
//		schedulerPas();
		...
		return new ModelAndView(
        "uw/input_pas_partner").addObject("cmd",pas);
