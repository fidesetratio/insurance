...
public class StatusPasFormController extends ParentFormController{
...
//    }
	...
	protected ModelAndView onSubmit( HttpServletRequest request, HttpServletResponse response, Object cmd, BindException errors ) throws Exception
    {
		...
		try{
			...
			int x = uwManager.prosesStatusPasFormController(pas,request,errors, action);
			if(x == 1){
        ...
        "uw/status_pas").addObject("cmd",pas);
	...
	protected Object formBackingObject(HttpServletRequest request)
		...
		if("DBD AGENCY".equals(pas.getJenis_pas()) || "DBD SYARIAH".equals(pas.getJenis_pas())){
			emailAdmin = uwManager.selectEmailCabangFromKodeAgen(pas.getMsag_id());
		}else{
			emailAdmin = uwManager.selectPasEmailAdminCabang(pas.getNo_kartu());
			
			if(emailAdmin == null || emailAdmin.equals("") || emailAdmin.isEmpty()){
		...
		pas.setEmailAgen(emailAgen);
		pas.setEmailAdmin(emailAdmin);
