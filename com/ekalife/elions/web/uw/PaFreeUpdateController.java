...
public class PaFreeUpdateController extends ParentFormController{
...
////			paTmmsFree.setLus_id(lus_id);
////			paTmmsFree.setLus_login_name(user.getLus_full_name());
		paTmmsFree = uwManager.updatePaTmmsFree(paTmmsFree);
		if(paTmmsFree.getTmms().getId() == null){
			request.setAttribute("successMessage","update gagal");
		}else{
			request.setAttribute("tmms_id",paTmmsFree.getTmms().getId());
			request.setAttribute("successMessage","update sukses");
		}
