...
public class InputDbdSyariahController extends ParentFormController{
	...
	protected void onBindAndValidate(HttpServletRequest request, Object cmd, BindException err) throws Exception {
		...
		//DBD SYARIAH INDIVIDU: LSBS_ID=209 & LSDBS_NUMBER=1
		pas.setProduk(1);//DBD SYARIAH INDIVIDU
		...
		//PAS
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "msp_pas_nama_pp", "","DBD Syariah : Nama Pemegang harus diisi");
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "msp_pas_tmp_lhr_pp", "","DBD Syariah : Tempat Lahir Pemegang harus diisi");
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "msp_pas_dob_pp", "","DBD Syariah : Tanggal Lahir Pemegang harus diisi");
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "msp_full_name", "","DBD Syariah : Nama Tertanggung harus diisi");
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "msp_pas_email", "","DBD Syariah : Alamat Email harus diisi");
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "msp_identity_no", "","DBD Syariah : No. Identitas harus diisi");
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "msp_pas_tmp_lhr_tt", "","DBD Syariah : Tempat Lahir Tertanggung harus diisi");
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "msp_date_of_birth", "","DBD Syariah : Tgl. Lahir Tertanggung harus diisi");
		if(pas.getLsag_id() == 6){
			ValidationUtils.rejectIfEmptyOrWhitespace(err, "msp_agama", "","DBD Syariah : Agama lainnya harus diisi");
		}
		if(pas.getMsp_occupation().equals("LAINNYA")){
			ValidationUtils.rejectIfEmptyOrWhitespace(err, "msp_occupation_else", "","DBD Syariah : Pekerjaan lainnya harus diisi");
		}
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "msp_postal_code", "","DBD Syariah : Kode Pos harus diisi");
		...
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "msp_address_1", "","DBD Syariah : Alamat harus diisi");
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "msp_area_code_rumah", "","DBD Syariah : Kode Area harus diisi");
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "msp_pas_phone_number", "","DBD Syariah : No. Telepon harus diisi");
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "msp_mobile", "","DBD Syariah : No. HP harus diisi");
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "msp_identity_no", "","DBD Syariah : No. Identitas harus diisi");
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "msp_city", "","DBD Syariah : Kota harus diisi");
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "lscb_id", "","DBD Syariah : Pembayaran harus diisi");
		ValidationUtils.rejectIfEmptyOrWhitespace(err, "msp_flag_cc", "","DBD Syariah : Cara Bayar harus diisi");
		...
		if(!(umurPp>=17 && umurPp<=85)){ //batas umur 17 s/d 85 tahun
			err.reject("","DBD Syariah : Umur pemegang minimal 17 tahun & maksimal 85 tahun");
		}
		if("58".equals(user.getLca_id())){
			if(!(umurTt>=1 && umurTt<=65)){ //batas umur 1 bulan s/d 65 tahun
				...
				if(umurTt < 1){
					if(umurBlnTt < 1){
						err.reject("","DBD Syariah : Umur tertanggung minimal 1 bulan & maksimal 65 tahun");
					}
				}else{
					err.reject("","DBD Syariah : Umur tertanggung minimal 1 bulan & maksimal 65 tahun");
				}
		...
		if(!Common.isEmpty(pas.getMsp_pas_email())){
			try {
				...
				err.reject("","DBD Syariah : email tidak valid");
			} finally {
		...
		try{
			...
			err.reject("","DBD Syariah : Kode Pos harus diisi angka");
		}
		if(!Common.isEmpty(pas.getMsp_pas_phone_number())){
			if(!Common.validPhone(pas.getMsp_pas_phone_number().trim())){
				err.reject("","DBD Syariah : No. Telepon harus diisi angka");
			}
		}
		if(!Common.isEmpty(pas.getMsp_pas_phone_number())){
			try{
				...
				if(Common.isEmpty(x)){
					err.reject("","DBD Syariah : No. Telepon tidak boleh diisi '0' atau '-'");
				}
			}catch(Exception e){
				err.reject("","DBD Syariah : No. Telepon tidak boleh diisi '0' atau '-'");
			}
...
//		}
		...
		List existedInput = uwManager.selectValidasiInputDbd(msp_pas_nama_pp, msp_full_name, msp_pas_beg_date);
		if(existedInput != null && existedInput.size() != 0){
			err.reject("","DBD Syariah : Nama ini sudah pernah di input");
		}
...
//		schedulerPas();
		...
		return new ModelAndView(
        "uw/input_dbd_syariah").addObject("cmd",pas);
