...
public class InputDbdController extends ParentFormController{
	...
	protected void onBindAndValidate(HttpServletRequest request, Object cmd, BindException err) throws Exception {
		...
		if("58".equals(user.getLca_id())){
			pas.setProduk(3);//DEMAM BERDARAH MALL
		}else{
			pas.setProduk(2);//DEMAM BERDARAH
		}
		...
		if("58".equals(user.getLca_id())){
			if(!(umurTt>=1 && umurTt<=60)){ //batas umur 1 bulan s/d 60 tahun
				//cek bulan
				if(umurTt < 1){
					if(umurBlnTt < 1){
						err.reject("","DBD : Umur tertanggung minimal 1 bulan & maksimal 60 tahun");
					}
				}else{
					err.reject("","DBD : Umur tertanggung minimal 1 bulan & maksimal 60 tahun");
				}
...
//		}
		...
		if(existedInput != null && existedInput.size() != 0){
			err.reject("","DBD : Nama ini sudah pernah di input");
		}
...
//		schedulerPas();
		...
		return new ModelAndView(
        "uw/input_dbd").addObject("cmd",pas);
