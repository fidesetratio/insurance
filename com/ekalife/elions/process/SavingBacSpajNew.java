...
public class SavingBacSpajNew extends ParentDao{
...
//					insert mst insured
					...
					proc_save_keluarga(edit, strTmpSPAJ);
					proc_save_pembayar_premi(edit,strTmpSPAJ);
...
//					}
//					sms_out.setText("Yth."+( (edit.getDatausulan().getJenis_pemegang_polis()==1?edit.getContactPerson().getMste_sex():edit.getPemegang().getMspe_sex())==1?"Bpk. ":"Ibu. ")+ edit.getPemegang().getMcl_first()+", Pengajuan Asuransi sedang kami proses. Utk info hub CS di 021-26508300. Apabila ada perubahan No. Telepon, HP dan alamat koresponden / penagihan, silahkan SMS ke nomor 0812 1111 880 – 0855 8079 403.");
...
//*/
			...
			if(edit.getPemegang().getFlag_upload()!=null){
				...
				Integer lsdbs_no = edit.getDatausulan().getLsdbs_number();
				...
				Integer questTemp = bacDao.selectCountQuestionaireTemp(no_temp);
				...
				if(MedquestTemp>0){
					...
					bacDao.updateMstAdditionalDescTemp(no_temp, edit.getPemegang().getReg_spaj()); //helpdesk [148055]
				}
//				if(benefTemp>0){
//					bacDao.copybenefTemp(no_temp,edit);
//				}
				if(ReffBiiTemp!=null && !(lsbs_id == 217 && lsdbs_no == 2)){
					bacDao.copyReffBiiTemp(no_temp,edit);
				}
				if(questTemp>0){
					bacDao.copyQuestionaireTemp(no_temp, edit);
				}
				if(edit.getPemegang().getFlag_upload()==3){	
					if(edit.getAgen().getMspe_email().trim()!=null){

					 	try{
						 	HashMap<String, String> mapEmail = uwDao.selectMstConfig(6, "Notifikasi E-SPAJ", "Notifikasi E-SPAJ");
...
//							String[] cc = mapEmail.get("NAME2")!=null?mapEmail.get("NAME2").toString().split(";"):null;
						    ...
						    String[] toEmailAgen = new String[] {edit.getAgen().getMspe_email().trim()+";"};
						    ...
						    EmailPool.send(me_id,"SMiLe E-Lions", 1, 1, 0, 0, 
									...
									toEmailAgen,//email agen 
									null, 
									bcc, 
									"[E-Lions] Notifikasi E-SPAJ NO VA: "+ edit.getTertanggung().getMste_no_vacc(), 
									"\nINFO UNTUK Agen Penutup: Data nasabah Atas nama : " +edit.getPemegang().getMcl_first()+" "+
									"berhasil di input dengan No SPAJ "+ FormatString.nomorSPAJ(strTmpSPAJ),							
									null,13);	
					 	}catch (Exception e) {
					 		EmailPool.send("E-Lions", 1, 0, 0, 0, null, 0, 0, new Date(), null, true, 
									props.getProperty("admin.ajsjava"), 
									new String[] { "randy@sinarmasmsiglife.co.id","eriza@sinarmasmsiglife.co.id" }, 
									null, 
									null, 
									"Error Kirim Email Notifikasi E-SPAJ NO VA", 
									"Error Kirim Email Notifikasi E-SPAJ NO VA ke Agen Penutup", 
									null, 
									null);
						}
				    }
					try{
						//copy file dari Folder temp ke Folder SPAJnya
						String lca_id = edit.getAgen().getLca_id();
						String source = props.getProperty("pdf.dir.file_gadget") + "\\" + no_temp+ "\\spaj";
						String destination = props.getProperty("pdf.dir.export") + "\\" + lca_id+ "\\" + strTmpSPAJ;
						...
						EmailPool.send("E-Lions", 1, 0, 0, 0, null, 0, 0, new Date(), null, true, 
								...
								null, 
								null, 
								"Error Generate File PDF dari Folder Temp Ke Folder SPAJ", 
								null, 
								null, 
								null);
					}
...
//				}
			...
			if(countBilling>0){
				...
				/*
				Double totalPremiNB = edit.getInvestasiutama().getDaftartopup().getPremi_berkala()+edit.getInvestasiutama().getDaftartopup().getPremi_tunggal()+edit.getDatausulan().getMspr_premium();
				Double totalPremiBilling = uwDao.selectTotalPremiNewBusiness(edit.getPemegang().getReg_spaj());
				if(totalPremiNB.equals(totalPremiBilling)){
					for(int i=1;i<=countBilling;i++){
						uwDao.update_billing(edit,i,countBilling);
					}
					   ...
					   // iii = new Integer(i);
					    while (intLevelId.intValue() < i)
					    {
					    	try {
							  ...
							  /*  //Note : Untuk Worksite , Program EBAM dimana level selain 4 (3,2,1 ) nutup, OR nya dapat
							    if (cabang.equalsIgnoreCase("42") && intLevelId.intValue() < i ){
							    	arrAgentRec[intCounter.intValue()].setComm_id(intLevelId.intValue());
							    }*/
...
//				}
//				
				edit.getTertanggung().setMkl_kerja(bacDao.selectKeteranganKerja(edit.getTertanggung().getMkl_kerja()));
...
//			
			...
			//pengiriman polis
			//Pekerjaan
			edit.getPemegang().setMkl_kerja(edit.getPemegang().getKerjaNote());
...
//			}
			...
			Integer counter=0;
			Map a = new HashMap();
			a.put("spaj", strTmpSPAJ);
			a.put("kyc", "3");
			a.put("kyc_pp", "1");
			delete("delete.mstKyc", a);	
			for(int i=0;i<pendapatanRutinBulan.length;i++){
				if(!pendapatanRutinBulan[i].contains("-")){
					insertKyc(strTmpSPAJ, counter++, "3", "1", pendapatanRutinBulan[i]);
				}
			}
		
			Map c = new HashMap();
			c.put("spaj", strTmpSPAJ);
			c.put("kyc", "5");
			c.put("kyc_pp", "1");
			delete("delete.mstKyc", c);		
			for(int i=0;i<tujuanInvestasi.length;i++){
				if(!tujuanInvestasi[i].contains("-")){
				insertKyc(strTmpSPAJ, counter++, "5", "1", tujuanInvestasi[i]);
				}
			}
		}
...
//				logger.info("update mst billing");
		...
		private void proc_save_rek_client(Cmdeditbac edit,String strTmpSPAJ)throws ServletException,IOException
		{
			...
			String kode_va = "";
			if(!StringUtil.isEmpty(edit.getTertanggung().getMste_no_vacc())){
...
//							insert mst rek client dan mst rek hist
		...
		private void proc_save_mst_policy(User currentUser, Cmdeditbac edit, String strPOClientID,String strTmpSPAJ,Date v_strDateNow , String v_strAgentId, String strAgentBranch,String strBranch,String strWilayah,String strRegion,String v_strregionid)throws ServletException,IOException
		{
			try {
				...
				//edit.getPemegang().setMspo_flag_spaj(2);
...
//				}
				...
				//tambah pilihan batch nonbatch untuk simas primelink (rider save). helpdesk [128634/128295] //Chandra
				if(!(edit.getDatausulan().getLsbs_id().equals(134) && edit.getDatausulan().getLsdbs_number().equals(10))){
					edit.getDatausulan().setPilih_batch("");
				}
				//helpdesk [137730] tambahin pilihan campaign
				if(edit.getDatausulan().getCampradio().equalsIgnoreCase("0")){
					edit.getDatausulan().setCampaign_id("");
				} else {
					if(edit.getDatausulan().getCampaign_id().equalsIgnoreCase("0~X0")){
						edit.getDatausulan().setCampaign_id("");
					}
				}
				...
				for(int i=0;i<pendapatanRutinBulan.length;i++){
					if(!pendapatanRutinBulan[i].contains("-")){
						insertKyc(strTmpSPAJ, counter++, "3", "2", pendapatanRutinBulan[i]);
					}
				}
				...
				for(int i=0;i<tujuanInvestasi.length;i++){
					if(!tujuanInvestasi[i].contains("-")){
					insertKyc(strTmpSPAJ, counter++, "5", "2", tujuanInvestasi[i]);
					}
				}
				
								
				//Ridhaal - update no_blanko menjadi Non Further setelah SPAJ di input
				basDao.updateMonitoringSpaj(edit.getPemegang().getMspo_no_blanko(), null, "update_further_to_NF_inputSpaj", null, null , null, null,null,null,strTmpSPAJ);
...
//					String sumnot = edit.getPemegang().getMkl_pendanaan().toUpperCase() != null ? "" : edit.getPemegang().getMkl_pendanaan().toUpperCase() ;
...
//					String sumnot2 = edit.getPemegang().getMkl_smbr_penghasilan().toUpperCase() != null ? "" :edit.getPemegang().getMkl_smbr_penghasilan().toUpperCase()  ;
...
//				
//					String sumnot = edit.getPemegang().getMkl_smbr_penghasilan().toUpperCase() == null ? "" :edit.getPemegang().getMkl_smbr_penghasilan().toUpperCase() ;
//				s.setKyc_desc(sumnot);
//					bacDao.insertMST_KYC(s);
//			}
//// *Insert SumberKyc Information to MST_KYC
		...
		private void proc_save_pesertax(Cmdeditbac edit, PesertaPlus_mix pesertaPlus_mix, String strTmpSPAJ)throws ServletException,IOException
		{
			...
			if(jumPtx_mix > 0){
				for (int i=0; i<jumPtx_mix.intValue(); i++)
				{
					...
					if("183,189,193,195,201,204,214,225".indexOf(plus2.getLsbs_id().toString())>-1 &&  plus2.getLsbs_id().intValue() < 300){
						NoUrut+=1;
						plus2.setNama(edit.getTertanggung().getMcl_first().toUpperCase());
					}else if("819,820,823,825,826,832,833,836,838,840,841,842,843,846,847,848".indexOf(plus2.getLsbs_id().toString())>-1){
						NoUrut+=1;
					}
...
//						plus2.setLsre_id(edit.getPemegang().getLsre_id());
					...
					if("183,189,193,195,201,204,214,225".indexOf(plus2.getLsbs_id().toString())>-1){
					...
					if("823,838,840,841,842,843,846,847,848".indexOf(plus2.getLsbs_id().toString())>-1){
						plus2.setMspo_provider(2);
					}
...
//				edit  address billing
	...
	private void insertKyc(String strTmpSPAJ,Integer noUrut,String kycId,String kycPp,String kycDesc)throws ServletException,IOException{
		Map kyc = new HashMap();
		kyc.put("reg_spaj", strTmpSPAJ);
		kyc.put("no_urut",noUrut);
		kyc.put("kyc_id",kycId);
		kyc.put("kyc_pp",kycPp);
		kyc.put("kyc_desc", kycDesc);
		bacDao.insertNewKyc(kyc);
	}
	
	private void proc_save_pembayar_premi(Cmdeditbac edit,String strTmpSPAJ) throws ServletException,IOException{
		...
		*/
			strPemPremiID = (String) querySingle("selectSequenceClientID", null);
			...
			if(calonPembayarpremi.equals("41")){
				...
				Integer rowupdated1 = update("updateNewClientPayerBadanHukum", edit.getPembayarPremi());
				if (rowupdated1 == 0){
					insert("insertNewClientPayerBadanHukum", edit.getPembayarPremi());
				}
	
				Integer rowupdated2 = update("updateAddressPemPremiIndividu", edit.getPembayarPremi());
				if (rowupdated2 == 0){
					insert("insertAddressPemPremiBadanHukum", edit.getPembayarPremi());
				}
	
				update("updateMstPolicyPayer", dataPolicy);
				...
				Integer rowupdated1 = update("updateNewClientPayerIndividu", edit.getPembayarPremi());
				if (rowupdated1 == 0){
					insert("insertNewClientPayerIndividu", edit.getPembayarPremi());
				}
	
				Integer rowupdated2 = update("updateAddressPemPremiIndividu", edit.getPembayarPremi());
				if (rowupdated2 == 0){
					insert("insertAddressPemPremiIndividu", edit.getPembayarPremi());
				}
		...
		Map d = new HashMap();
		d.put("spaj", strTmpSPAJ);
		d.put("kyc", "6");
		d.put("kyc_pp", "0");
		delete("delete.mstKyc", d);	
		if (edit.getPembayarPremi().getMkl_kerja_other_radio().equals("1")){
			insertKyc(strTmpSPAJ,counter++,"6","0", edit.getPembayarPremi().getMkl_kerja_other());
		}
		
		Map e = new HashMap();
		e.put("spaj", strTmpSPAJ);
		e.put("kyc", "7");
		e.put("kyc_pp", "0");
		delete("delete.mstKyc", e);	
		...
		if (!edit.getPembayarPremi().getTotal_rutin().equals("")){
			insertKyc(strTmpSPAJ,counter++,"7","0", edit.getPembayarPremi().getTotal_rutin());
		}
		
		Map f = new HashMap();
		f.put("spaj", strTmpSPAJ);
		f.put("kyc", "8");
		f.put("kyc_pp", "0");
		delete("delete.mstKyc", f);	
		...
		if (!edit.getPembayarPremi().getTotal_non_rutin().equals("")){
			insertKyc(strTmpSPAJ,counter++,"8","0", edit.getPembayarPremi().getTotal_non_rutin());
		}
		...
		for(int i=0;i<pendapatanRutinBulan.length;i++){
			if(!pendapatanRutinBulan[i].contains("-")){
				insertKyc(strTmpSPAJ, counter++, "3", "0", pendapatanRutinBulan[i]);
			}
		}
		
		Map b = new HashMap();
		b.put("spaj", strTmpSPAJ);
		b.put("kyc", "4");
		b.put("kyc_pp", "0");
		delete("delete.mstKyc", b);
		for(int i=0;i<pendapatanNonTahun.length;i++){
			if(!pendapatanNonTahun[i].contains("-")){
				insertKyc(strTmpSPAJ, counter++, "4", "0", pendapatanNonTahun[i]);
			}
		}
		...
		for(int i=0;i<tujuanInvestasi.length;i++){
			if(!tujuanInvestasi[i].contains("-")){
			insertKyc(strTmpSPAJ, counter++, "5", "0", tujuanInvestasi[i]);
			}
		}
		
	}
	
	private void proc_save_keluarga(Cmdeditbac edit,String strTmpSPAJ) throws ServletException,IOException{
		Calendar tgl_sekarang = Calendar.getInstance(); 
		...
		if(edit.getDatausulan().getJenis_pemegang_polis()==0){
			if(!edit.getPemegang().getPemegang_polis().equals("2")){
				if (edit.getPemegang().getPemegang_polis().equals("0")){
					...
					int rowupdated = update("updateMstKeluarga", paramSuami);
					if(rowupdated == 0) insert("insert_mst_keluarga", paramSuami);
					...
					int rowupdated2 = update("updateMstKeluarga", paramAyah);
					if(rowupdated2 == 0) insert("insert_mst_keluarga", paramAyah);
					...
					int rowupdated3 = update("updateMstKeluarga", paramIbu);
					if(rowupdated3 == 0) insert("insert_mst_keluarga", paramIbu);
				}
			}
		}
	}
