...
public class Eduvest {
    protected final Log logger = LogFactory.getLog( getClass() );
	private static Eduvest instance;
	
	public static Eduvest getInstance()
	{
		if( instance == null )
		{
			instance = new Eduvest();
		}
		return instance;
	}
	...
	public PdfContentByte createPdf( BacSpajParamVO paramVO )
	{
		...
		over.showTextAligned(Element.ALIGN_LEFT, pemegang.getMcl_first(), x-367, y-100, 0);
...
//    	over.showTextAligned(Element.ALIGN_LEFT, pemegang.getMspe_mother(), x-265, y-93, 0);
    	...
    	if (jns_identitas != null)
    	{
	    	...
	    	{
	    		over.showTextAligned(Element.ALIGN_LEFT, "X", x-363, y-121, 0);
	    	}else{
	    		over.showTextAligned(Element.ALIGN_LEFT, "X",  x-320, y-121,0);
	    		over.showTextAligned(Element.ALIGN_LEFT, jenis_identitas, x-276, y-121, 0);
	    		}
    		}
    	...
    	if (no_identitas != null)
    	{
	    	...
	    	{
    		over.showTextAligned(Element.ALIGN_LEFT, String.valueOf(no_identitas.charAt(i)), x-363+(i*13), y-142, 0);
	    	}
    	}
    	...
    	if (tanggal_lahir != null)
    	{
	    	...
	    	{
	    		over.showTextAligned(Element.ALIGN_LEFT, String.valueOf(tgl_lhr.charAt(i)), x-342+(i*10), y-161, 0);
	    	}
	    			
	    	for (int i =0 ; i< bln_lhr.length() ; i++)
	    	{
	    		over.showTextAligned(Element.ALIGN_LEFT, String.valueOf(bln_lhr.charAt(i)), x-288+(i*12),  y-161, 0);
	    	}
	    			
	    	for (int i =0 ; i< thn_lhr.length() ; i++)
	    	{
	    		over.showTextAligned(Element.ALIGN_LEFT, String.valueOf(thn_lhr.charAt(i)), x-228+(i*13),  y-161, 0);
	    	}
    	}
    		
    		over.showTextAligned(Element.ALIGN_LEFT, pemegang.getMspe_place_birth(), x-153,  y-161, 0);
    	...
    	if (jenis_kelamin !=null)
    	{
	    	...
	    	{
	    		over.showTextAligned(Element.ALIGN_LEFT, "X", x-318, y-180, 0);// pria 363
	    	}else{
	    		over.showTextAligned(Element.ALIGN_LEFT, "X", x-363, y-180, 0);// wanita
	    	}
    	}
    	...
    	if (sts_marital != null)
    	{
	    	...
	    	{
	    		case 1:
	    			over.showTextAligned(Element.ALIGN_LEFT, "X", x-218, y-180, 0);//single //218
	    			break;
	    		case 2:
	    			over.showTextAligned(Element.ALIGN_LEFT, "X", x-146, y-180, 0);//menikah
	    			break;
	    		case 3:
	    			over.showTextAligned(Element.ALIGN_LEFT, "X", x-94, y-180, 0);//janda
	    			break;
	    		case 4:
	    			over.showTextAligned(Element.ALIGN_LEFT, "X", x-94, y-180, 0);//duda
	    			break;
	    			}
    			}				    			
    	...
    	if (id_agama != null)
    	{
	    	switch (id_agama)
	    		{
	    			case 1:
	    					over.showTextAligned(Element.ALIGN_LEFT, "X", x-363, y-200, 0);//islam
	    					break;
	    			case 2:
	    					over.showTextAligned(Element.ALIGN_LEFT, "X", x-318, y-200, 0);//protestan
	    					break;
	    			case 3:
	    					over.showTextAligned(Element.ALIGN_LEFT, "X", x-257, y-200, 0);//katolik
	    					break;
	    			case 5:
	    					over.showTextAligned(Element.ALIGN_LEFT, "X", x-198, y-200, 0);//hindu
	    						break;
	    			case 4:
	    					over.showTextAligned(Element.ALIGN_LEFT, "X", x-146, y-200, 0);//budha x-146, y-200
	    					break;
	    			}
    			}
    	...
    	if (nama_negara != null)
    	{
	    	...
	    	{
	    		over.showTextAligned(Element.ALIGN_LEFT, "X", x-363, y-220, 0);//x-363, y-220
	    	}else{
	    		over.showTextAligned(Element.ALIGN_LEFT, "X", x-297, y-220, 0);
	    		over.showTextAligned(Element.ALIGN_LEFT, nama_negara, x-175, y-220, 0);
	    	}
    	}
    	...
    	if (pendidikan != null)
    	{
	    	...
	    	{
	    		case 8:
	    			over.showTextAligned(Element.ALIGN_LEFT, "X",x-93, y-243, 0);//sd
	    			break;
	    		case 9:
	    			over.showTextAligned(Element.ALIGN_LEFT, "X", x-318, y-243, 0);//smp
	    			break;
	    		case 1:
	    			over.showTextAligned(Element.ALIGN_LEFT, "X", x-364, y-243, 0);//SD
	    			break;
	    		case 3:
	    			over.showTextAligned(Element.ALIGN_LEFT, "X", x-273, y-243, 0);//smu
	    			break;
	    		case 4:
	    			over.showTextAligned(Element.ALIGN_LEFT, "X", x-173, y-243, 0);//s1  x-173, y-243
	    			break;
	    		case 5:
	    			over.showTextAligned(Element.ALIGN_LEFT, "X", x-226, y-243, 0);//d1/d3
	    			break; 
	    			default:
	    				over.showTextAligned(Element.ALIGN_LEFT, "X", x-93, y-243, 0);
	    				over.showTextAligned(Element.ALIGN_LEFT, pendidikan, x-47, y-243, 0);
	    			break;
	    			}
    	...
    	over.showTextAligned(Element.ALIGN_LEFT, alamat_rumah, x-366, y-263, 0);
    	...
    	if ( kode_pos_rumah!=null)
			{
    			...
    			{
    				over.showTextAligned(Element.ALIGN_LEFT, String.valueOf(kode_pos_rumah.charAt(i)), x-363+(i*13),y-289, 0);
    			}
			}
    	...
    	if (area_rumah != null)
    	{
    		for(int i=0;i<area_rumah.length();i++){
    			over.showTextAligned(Element.ALIGN_LEFT, String.valueOf(area_rumah.charAt(i)), x-266+(i*13),y-289, 0);
      		}
    	}
    	...
    	if (telp_rumah != null)
    	{
    		for(int i=0;i<telp_rumah.length();i++){
    			over.showTextAligned(Element.ALIGN_LEFT, String.valueOf(telp_rumah.charAt(i)), x-199+(i*13),y-289, 0);
      		}
    	}
    	...
    	if (no_hp != null)
    	{
	    	...
	    	{
	    		over.showTextAligned(Element.ALIGN_LEFT, String.valueOf(no_hp.charAt(i)), x-363+(i*13),y-311, 0);
	    	}
    	}
    	String imel= tertanggung.getEmail();
    	over.showTextAligned(Element.ALIGN_LEFT,imel, x-145, y-312, 0);
    	String penghasilan = pemegang.getMkl_penghasilan();
    	if (penghasilan != null)
    	{
	    if (penghasilan.indexOf("<= RP. 10 JUTA") >= 0 )
	    	{
	    	over.showTextAligned(Element.ALIGN_LEFT, "X", x-464, y-340, 0);
	    }else if (penghasilan.indexOf("> RP. 10 JUTA - RP. 50 JUTA") >= 0)
	    	{
	    	over.showTextAligned(Element.ALIGN_LEFT, "X", x-463, y-353, 0);
	    }else if (penghasilan.indexOf("> RP. 50 JUTA - RP. 100 JUTA") >= 0)
	    	{
    		over.showTextAligned(Element.ALIGN_LEFT, "X", x-325, y-340, 0);//x-325, y-339
	    }else if (penghasilan.indexOf("> RP. 100 JUTA - RP. 300 JUTA") >= 0)
		    {
	    	over.showTextAligned(Element.ALIGN_LEFT, "X", x-325, y-353, 0);
	    }else if (penghasilan.indexOf("> RP. 300 JUTA - RP. 500 JUTA") >= 0)
		    {
		    over.showTextAligned(Element.ALIGN_LEFT, "X", x-192, y-340, 0);
		}else if (penghasilan.indexOf("> RP. 500 JUTA") >=0)
		    {
		    over.showTextAligned(Element.ALIGN_LEFT, "X", x-192, y-353, 0); //x-76, y-375
		    }
    	}
...
//    	over.showTextAligned(Element.ALIGN_LEFT, pekerjaan, x-366, y-365, 0);
    	...
    	over.showTextAligned(Element.ALIGN_LEFT, uraian_kerja, x-366, y-369, 0);
    	...
    	over.showTextAligned(Element.ALIGN_LEFT,  nama_bank , x-151, y-397, 0);
    	String cabang = dataagen.getLsrg_nama();
		over.showTextAligned(Element.ALIGN_LEFT, cabang , x-151,y-410, 0);
...
//		over.showTextAligned(Element.ALIGN_LEFT, kota , x-151,y-423, 0);
    	...
    	over.showTextAligned(Element.ALIGN_LEFT,  no_rek , x-366, y-410, 0);
    	String atas_nama = data_rek.getMrc_nama();
    	over.showTextAligned(Element.ALIGN_LEFT,  atas_nama ,x-366, y-423, 0);
    	/*
    	...
    	//data tertanggung
    	over.showTextAligned(Element.ALIGN_LEFT, tertanggung.getMcl_first(), x-366, y-460, 0);
    	over.showTextAligned(Element.ALIGN_LEFT, tertanggung.getMspe_mother(), x-366, y-475, 0);
    	jenis_kelamin = tertanggung.getMspe_sex();
		if (jenis_kelamin !=null)
		{
			if (jenis_kelamin.intValue() == 0)
			{
				over.showTextAligned(Element.ALIGN_LEFT, "X", x-17, y-460 ,0);//lekong=co  x-62, y-460
			}else{
				over.showTextAligned(Element.ALIGN_LEFT, "X", x-62, y-460, 0);//perempuan
			}
		}
    	...
    	if (tanggal_lahir != null)
    	{
	    	...
	    	for (int i =0 ; i< tgl_lhr.length() ; i++)
	    	{
	    		over.showTextAligned(Element.ALIGN_LEFT, String.valueOf(tgl_lhr.charAt(i)), x-342+(i*10), y-488, 0);
	    	}
	    			
	    	for (int i =0 ; i< bln_lhr.length() ; i++)
	    	{
	    		over.showTextAligned(Element.ALIGN_LEFT, String.valueOf(bln_lhr.charAt(i)), x-288+(i*12),  y-488, 0);
	    	}
	    			
	    	for (int i =0 ; i< thn_lhr.length() ; i++)
	    	{
	    		over.showTextAligned(Element.ALIGN_LEFT, String.valueOf(thn_lhr.charAt(i)), x-228+(i*13),  y-488, 0);
	    	}
    	}
    	over.showTextAligned(Element.ALIGN_LEFT, tertanggung.getMspe_place_birth(), x-153,  y-488, 0);
    	alamat_rumah = tertanggung.getAlamat_rumah();
    	over.showTextAligned(Element.ALIGN_LEFT, alamat_rumah, x-366, y-503, 0);
		...
		if (mulai_tanggung != null)
		{
			...
			{
				over.showTextAligned(Element.ALIGN_LEFT, String.valueOf(tanggal_beg_date.charAt(i)), x-393+(i*12), y-563, 0);
			}
			
			for (int i =0 ; i< bulan_beg_date.length() ; i++)
			{
				over.showTextAligned(Element.ALIGN_LEFT, String.valueOf(bulan_beg_date.charAt(i)), x-339+(i*12), y-563, 0);
			}
			
			for (int i =0 ; i<tahun_beg_date.length() ; i++)
			{
				over.showTextAligned(Element.ALIGN_LEFT, String.valueOf(tahun_beg_date.charAt(i)), x-280+(i*12), y-563, 0);
			}
		}
...
//		}
		...
		over.showTextAligned(Element.ALIGN_LEFT, FormatString.formatCurrency(krs, new BigDecimal(premi.doubleValue())) , x-341, y-578, 0);
...
//				over.showTextAligned(Element.ALIGN_LEFT,  jBene, x-365,y-678-(i*10), 0);		
				over.showTextAligned(Element.ALIGN_LEFT,  nama_benef , x-460,y-678-(i*15), 0);		
				over.showTextAligned(Element.ALIGN_LEFT,  tanggal_lahir_benef , x-250, y-(678+ (i*15)), 0);
				over.showTextAligned(Element.ALIGN_LEFT,  hub_benef , x-140,y-(678+ (i*15)), 0);
				over.showTextAligned(Element.ALIGN_LEFT,  Double.toString(persen_benef) , x-20,y-(678+ (i*15)), 0);
			}
		}
...
//    			data datausulan
    			...
    			*/
	    		...
	    		over.showTextAligned(Element.ALIGN_LEFT,  kode_agen , x-425,y-692, 0);
	    		over.showTextAligned(Element.ALIGN_LEFT,  nama_agen , x-425,y-705, 0);
	    		over.showTextAligned(Element.ALIGN_LEFT,  pemegang.getMcl_first() , x-170,y-577, 0);
	    		over.showTextAligned(Element.ALIGN_LEFT,  tertanggung.getMcl_first() , x-325,y-577, 0);
