...
import com.ekalife.elions.model.CplanDet;
...
public class PrintPolisPaBsm {
	
	protected static final Log logger = LogFactory.getLog( PrintPolisPaBsm.class );
...
/*	public static void main(String args[]) {
	...
	public Boolean generatePaBsm(String outputName, Cplan cplan, CplanDet cplanDet, Date sysdate, String hamid){
...
//			String mspo_policy_no = (String) dataAdmedika.get("MSPO_POLICY_NO");
			String mspo_policy_no_format = cplan.getNo_sertifikat();
//			String msen_endors_no = (String) dataAdmedika.get("MSEN_ENDORS_NO");
			String nama_pp = (String) cplan.getFull_name();
			String nama_tt = (String) cplanDet.getNama_peserta();
			Date beg_date = (Date) cplan.getMcp_beg_date();
			Date end_date = (Date) cplan.getMcp_end_date();
			String alamat = (String) cplan.getAddress1();
			String kota = (String) cplan.getCity();
			String kode_pos = (String) cplan.getPostal_code();
			int usiaPp = cplan.getMcp_umur();
			int usiaTt = cplanDet.getUmur();
			int plan = cplanDet.getPlan();
			String manfaat = "";
		
			if(plan == 1){
				manfaat = "Personal Accident Risiko A";
			}else if(plan == 2){
				manfaat = "Personal Accident Risiko AB";
			}else if(plan == 3){
				manfaat = "Personal Accident Risiko ABD";
			}
...
//			 adding some metadata
			HashMap moreInfo = new HashMap();
			moreInfo.put("Author", "PT ASURANSI JIWA SINARMAS MSIG Tbk.");
			moreInfo.put("Title", "PA BSM");
			moreInfo.put("Subject", "Polis Personal Accident");
...
//			if(syariah){
				reader = new PdfReader("\\\\ebserver\\pdfind\\Template\\PaBsm\\SSUPABsm.pdf");
//			}else 
			...
			over.showTextAligned(PdfContentByte.ALIGN_CENTER, "POLIS PERSONAL ACCIDENT", 299, 760, 0);
			...
			//kiri
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "No. Polis", 30, 735, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "Pemegang Polis", 30, 722, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "Tgl. Lahir / Umur", 30, 709, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "Masa Asuransi", 30, 696, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "Alamat", 30, 683, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, ":     "+mspo_policy_no_format, 130, 735, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, ":     "+nama_pp.toUpperCase(), 130, 722, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, ":     "+FormatDate.toIndonesian(cplan.getMcp_tgl_lahir())+" / "+usiaPp+" Tahun", 130, 709, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, ":     "+"1 tahun dan dapat diperpanjang", 130, 696, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, ":     ", 130, 683, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, alamat, 30, 670, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, kota +" - "+kode_pos, 30, 657, 0);
			//kanan
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "Tertanggung", 299, 735, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "Tgl. Lahir / Umur", 299, 722, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "Uang Pertanggungan", 299, 709, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "Pilihan Manfaat", 299, 696, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "Premi", 299, 683, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, ":     "+nama_tt.toUpperCase(), 399, 735, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, ":     "+FormatDate.toIndonesian(cplanDet.getBod_peserta())+" / "+usiaTt+" Tahun", 399, 722, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, ":     "+FormatString.formatCurrency("Rp. ", new BigDecimal(cplan.getLump_sum())) + ",-", 399, 709, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, ":     "+manfaat, 399, 696, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, ":     "+FormatString.formatCurrency("Rp. ", new BigDecimal(cplanDet.getPremi())) + ",-" + " per TAHUN", 399, 683, 0);
			...
			over = stamp.getOverContent(1);
			over.beginText();
			over.setFontAndSize(arial_narrow_bold,10);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "      _____________", 399, 575, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "       Chief Operating & IT Officer", 399, 564, 0);
			over.endText();
			
			Image img = Image.getInstance(hamid);
			img.setAbsolutePosition(415, 586);		
			img.scaleAbsolute(120, 34);
			over.addImage(img,img.getScaledWidth(), 0, 0, img.getScaledHeight(), 415, 586);
			over.stroke();
...
//			}
	...
	public Boolean generatePaBsm_free(String outputName, Cplan cplan, CplanDet cplanDet, Date sysdate, String hamid){
		Boolean result = false;
		try{
...
//			}else 
			...
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, ":     "+nama_pp.toUpperCase(), 130, 722, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, ":     "+FormatDate.toIndonesian(cplan.getMcp_tgl_lahir())+" / "+usiaPp+" Tahun", 130, 709, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, ":     "+FormatDate.toIndonesian(beg_date) + " s/d " + FormatDate.toIndonesian(end_date), 130, 696, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, ":     ", 130, 683, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, alamat, 30, 670, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, kota +" - "+kode_pos, 30, 657, 0);
			//kanan
			...
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "Uang Pertanggungan", 299, 709, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "Pilihan Manfaat", 299, 696, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "Premi Tahunan", 299, 683, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, ":     "+nama_tt.toUpperCase(), 399, 735, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, ":     "+FormatDate.toIndonesian(cplanDet.getBod_peserta())+" / "+usiaTt+" Tahun", 399, 722, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, ":     "+FormatString.formatCurrency("Rp. ", new BigDecimal(cplan.getLump_sum())) + ",-", 399, 709, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, ":     "+manfaat, 399, 696, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, ":     Rp. 36.364,-*", 399, 683, 0);
			
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "      Jakarta, "+FormatDate.toIndonesian(sysdate), 399, 650, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "      PT Asuransi Jiwa Sinarmas MSIG Tbk.", 399, 641, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "      Andrew Bain", 399, 585, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "*Premi dibebankan kepada PT Asuransi Jiwa Sinarmas MSIG Tbk.", 30, 564, 0);
			over.endText();
			
			over = stamp.getOverContent(1);
			over.beginText();
			over.setFontAndSize(arial_narrow_bold,10);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "      _____________", 399, 585, 0);
			over.showTextAligned(PdfContentByte.ALIGN_LEFT, "       Chief Operating & IT Officer", 399, 574, 0);
			over.endText();
			
			Image img = Image.getInstance(hamid);
			img.setAbsolutePosition(415, 596);		
			img.scaleAbsolute(120, 34);
			over.addImage(img,img.getScaledWidth(), 0, 0, img.getScaledHeight(), 415, 596);
			over.stroke();
