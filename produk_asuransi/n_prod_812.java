...
//n_prod_812 (TOTAL AND PERMANENT DISABILITY RIDER)
/*
 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class n_prod_812 extends n_prod{
...
//	of_set_usia_tt
	...
	public double of_get_rate1( int li_class, int flag_jenis_plan, int nomor_bisnis, int umurttg, int umurpp)
	{
		...
		try {
			...
			Double result = 0.0;
			if(ii_bisnis_no==7){
				result = query.selectRateRider(is_kurs_id, umurttg, 0, 812, ii_bisnis_no);
			}else{
				result = query.selectRateRider(is_kurs_id, umurttg, 0, 812, 1);
			}
			if(result != null) {
	...
	public void count_rate(int klas, int unit ,int kode_produk, int nomor_produk, String kurs, int umurttg, int umurpp, double up, double premi, int pmode, int flag, int ins_period,int payperiod)
	{
		...
		try{
			...
			produk2.count_rate_812(klas,unit,812,nomor_produk,kurs,umurttg,umurpp,up,premi,pmode,flag,ins_period,payperiod);
			mbu_jumlah =  produk2.ldec_temp3;
			mbu_rate = produk2.ldec_rate3;
	...
	public double cek_maks_up_rider (Double up_rider, String kurs)
	{
		...
		{
			if (up_rider.doubleValue() > max_up)
			{
				up_rider = new Double(max_up);
			}
		}else{
			if (up_rider.doubleValue() > max_up)
			{
