package produk_asuransi;
import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import com.ekalife.utils.f_hit_umur;

//n_prod_848 (Smile Medical Extra (848-1~70) helpdesk [129135] //Chandra)

public class n_prod_848 extends n_prod{
	
	protected final Log logger = LogFactory.getLog( getClass() );
	
	Query query = new Query();
	f_hit_umur umr = new f_hit_umur();
	public n_prod_848()
	{
		ii_bisnis_id = 848;
		ii_contract_period = 1;
		ii_age_from = 1;
//		ii_age_to = 65;
		ii_age_to = 64; //20200511, spek update max 64 tahun
		
		indeks_is_forex=1;
		is_forex= new String[indeks_is_forex];
		is_forex[0]="01";	
		
		indeks_ii_pmode_list=5;
		ii_pmode_list = new int[indeks_ii_pmode_list];	
		ii_pmode_list[1] = 3;   //tahunan
		ii_pmode_list[2] = 2;   //semester
		ii_pmode_list[3] = 1;   //Tri		
		ii_pmode_list[4] = 6;   //bulanan	

		indeks_idec_pct_list=7;
		idec_pct_list = new double[indeks_idec_pct_list];
		idec_pct_list[0] = 1;     // pmode 0
		idec_pct_list[1] = 0.270; // pmode 1
		idec_pct_list[2] = 0.525; // pmode 2
		idec_pct_list[3] = 1;     // pmode 3
		idec_pct_list[4] = 1 ;    // pmode 4
		idec_pct_list[5] = 1 ;    // pmode 5
		idec_pct_list[6] = 0.1; // pmode 6	
		flag_rider=1;
		li_sd = 75;
		simas = 1;
	}

	public String of_check_usia(int tahun1, int bulan1, int tanggal1,int tahun2, int bulan2, int tanggal2,int lm_byr,int nomor_produk) {
			
		if (ii_usia_pp > 90) {
			hsl = "Usia Pemegang Polis maximum : 90 Tahun !!!"; 
		} else if (ii_usia_pp < 17) {
			hsl = "Usia Pemegang Polis minimum : 17 Tahun !!!";
		}
		return hsl;		
	}	

//	of_set_usia_pp
	 public void of_set_usia_pp(int ai_pp)
	 {
		 ii_usia_pp = ai_pp;
	 }

//	of_set_usia_tt
	 public void of_set_usia_tt(int ai_tt)
	 {
		 ii_usia_tt = ai_tt;
	 }

	public String of_check_usia_rider(int tahun1, int bulan1, int tanggal1,int tahun2, int bulan2, int tanggal2,int lm_byr,int nomor_produk, int nomor_rider) 
	{
		
		hsl="";
		f_hit_umur umr =new f_hit_umur();
		int hari=umr.hari(tahun1, bulan1, tanggal1, tahun2, bulan2, tanggal2);
		
		if (hari < 15)
		{
			hsl="Usia Masuk untuk SMiLe Medical Extra minimum 15 Hari";
		}		

		if (ii_usia_tt > ii_age_to)
		{
			hsl="Usia Masuk untuk SMiLe Medical Extra maximum : " + ii_age_to +" tahun";
		}
		return hsl;		
	}	
	
	public String of_check_usia_kesehatan(int utama, int hub, int tahun1, int bulan1, int tanggal1, int tahun2, int bulan2, int tanggal2, int lm_byr,int nomor_produk, int nomor_rider) 
	{
		hsl="";
		f_hit_umur umr =new f_hit_umur();
		int hari=umr.hari(tahun1, bulan1, tanggal1, tahun2, bulan2, tanggal2);

		if(utama == 0)
			ii_age_from = 0;
		else
			ii_age_from = 17;		
		
		if(hub != 4 && hub != 8 && hub != 21){
			if(ii_usia_tt < ii_age_from){
				hsl="Usia Masuk untuk Suami/Istri dan Orang Tua SMiLe Medical Extra minimum : " + ii_age_from+ " tahun";
			}else if (ii_usia_tt > ii_age_to){
				hsl="Usia Masuk untuk Suami/Istri dan Orang Tua SMiLe Medical Extra maximum : " + ii_age_to +" tahun";
			}
		}else{//anak kandung, angkat & tiri
			int umur_ttg_max = ii_age_to;
			if(hub == 4 || hub == 8 || hub == 21)
				umur_ttg_max = 24;
			
			if(ii_usia_tt < ii_age_from){
				hsl="Usia Masuk untuk Anak Kandung, Anak Angkat, atau Anak Tiri SMiLe Medical Extra minimum : " + ii_age_from+ " tahun";
			}else if (ii_usia_tt > umur_ttg_max){
				hsl="Usia Masuk untuk Anak Kandung, Anak Angkat, atau Anak Tiri SMiLe Medical Extra maximum : " + umur_ttg_max +" tahun";
			}
		}
		if (hari < 15)
		{
			hsl="Usia Masuk untuk SMiLe Medical Extra minimum 15 Hari";
		}		
		
//		if(ii_usia_tt < ii_age_from){
//			hsl="Usia Masuk untuk SMiLe Medical Extra minimum : " + ii_age_from+ " tahun";
//		}else if (ii_usia_tt > ii_age_to){
//			hsl="Usia Masuk untuk SMiLe Medical Extra maximum : " + ii_age_to +" tahun";
//		}
		return hsl;		
	}
	
	//helpdesk [145625] tambah rider smile medical extra untuk produk agency
	public String of_check_usia_kesehatan2(int utama, int hub, int umur_ttg_tamb, int tahun1, int bulan1, int tanggal1, int tahun2, int bulan2, int tanggal2, int lm_byr,int nomor_produk, int nomor_rider) 
	{
		hsl="";
		f_hit_umur umr =new f_hit_umur();
		int hari=umr.hari(tahun1, bulan1, tanggal1, tahun2, bulan2, tanggal2);

		if(utama == 0)
			ii_age_from = 0;
		else
			ii_age_from = 17;		
		
		if(hub != 4 && hub != 8 && hub != 21){
			if(ii_usia_tt < ii_age_from){
				hsl="Usia Masuk untuk Suami/Istri dan Orang Tua SMiLe Medical Extra minimum : " + ii_age_from+ " tahun";
			}else if (ii_usia_tt > ii_age_to){
				hsl="Usia Masuk untuk Suami/Istri dan Orang Tua SMiLe Medical Extra maximum : " + ii_age_to +" tahun";
			}
		}else{//anak kandung, angkat & tiri
			int umur_ttg_max = ii_age_to, umur_min_hari = 15;
			if(utama != 0 && (hub == 4 || hub == 8 || hub == 21))
				umur_ttg_max = 24;

			if(umur_ttg_tamb < umur_min_hari)
				hsl="Usia Masuk untuk Anak Kandung, Anak Angkat, atau Anak Tiri SMiLe Medical Extra minimum : " + umur_min_hari + " hari";
//			if(ii_usia_tt < ii_age_from)
//				hsl="Usia Masuk untuk Anak Kandung, Anak Angkat, atau Anak Tiri SMiLe Medical Extra minimum : " + ii_age_from+ " tahun";
			if (ii_usia_tt > umur_ttg_max){
				hsl="Usia Masuk untuk Anak Kandung, Anak Angkat, atau Anak Tiri SMiLe Medical Extra maximum : " + umur_ttg_max +" tahun";
			}
		}
		if (hari < 15)
		{
			hsl="Usia Masuk untuk SMiLe Medical Extra minimum 15 Hari";
		}	
		
		return hsl;		
	}
	
	public double of_get_rate1( int li_class, int flag_jenis_plan, int nomor_bisnis, int umurttg, int umurpp)
	{
		String err="";
		rate_rider=0;
		try {
			Double result = query.selectRateRider(is_kurs_id, umurttg, 0, 848, nomor_bisnis);
			if(result != null) {
				rate_rider = result.doubleValue();		  	
			}
		}
		catch (Exception e) {
			err=(e.toString());
	  	} 		  	

		return rate_rider;	
	}
	
	public void wf_set_premi(int tahun,int bulan,int tanggal, int cara_bayar,int tahun_1,int bulan_1,int tanggal_1,int insperiod, int flag_jenis_plan, int ii_age, int lama_bayar, int flag_cerdas_siswa, int umurpp,int kode_bisnis,int number_bisnis)
	{	
		li_insured = li_sd - ii_age;		
		tanggal_sementara = umr.f_add_months(tahun,bulan,tanggal,li_insured * 12);
		ldt_edate.set(tanggal_sementara.getYear()+1900,tanggal_sementara.getMonth(),tanggal_sementara.getDate());
		ldt_edate.add(ldt_edate.DATE,-1);
	
		li_insured = li_sd - ii_age;
		tanggal_sementara1=umr.f_add_months(tahun,bulan,tanggal,( li_insured * 12)-1 );
		//ldt_epay = f_add_months(istr_polis.beg_date, ( li_insured * 12) - 1)
		ldt_epay.set(tanggal_sementara1.getYear()+1900,tanggal_sementara1.getMonth(),tanggal_sementara1.getDate());

		if (is_kurs_id.equalsIgnoreCase("01"))
		{
			li_kali = 1000;
		}
		li_insured = li_sd - ii_age;
	}
	
	public void wf_set_premi2(int lsre_id, int peserta, int tahun,int bulan,int tanggal, int cara_bayar,int tahun_1,int bulan_1,int tanggal_1,int insperiod, int flag_jenis_plan, int ii_age, int lama_bayar, int flag_cerdas_siswa, int umurpp,int kode_bisnis,int number_bisnis){
		int umur_max = li_sd;
		if(peserta != 0 && (lsre_id == 4 || lsre_id == 8 || lsre_id == 21)){
			umur_max = 25;
		} 
		
		li_insured = umur_max - ii_age;	
		
		tanggal_sementara = umr.f_add_months(tahun,bulan,tanggal,li_insured * 12);
		ldt_edate.set(tanggal_sementara.getYear()+1900,tanggal_sementara.getMonth(),tanggal_sementara.getDate());
		ldt_edate.add(ldt_edate.DATE,-1);

//		li_insured = umur_max - ii_age;
		tanggal_sementara1=umr.f_add_months(tahun,bulan,tanggal,( li_insured * 12)-1 );
		//ldt_epay = f_add_months(istr_polis.beg_date, ( li_insured * 12) - 1)
		ldt_epay.set(tanggal_sementara1.getYear()+1900,tanggal_sementara1.getMonth(),tanggal_sementara1.getDate());

		if (is_kurs_id.equalsIgnoreCase("01"))
		{
			li_kali = 1000;
		}
//		li_insured = umur_max - ii_age;
	}
	
	public void of_set_pmode(int ai_pmode)
	{
		ii_pmode = ai_pmode;		
	}		
	
	public double hit_premi_rider(double rate,double up,double persen, double premi)
	{
		double diskon = new Double(0.975);
		if(ii_bisnis_no > 10){
			premi = rate * diskon * persen;
		}else{
			premi = rate * persen;
		}
		return premi;	
	}

	public double of_get_up(double idec_premi,double idec_up ,int li_unit, int flag_jenis_plan, int kode_bisnis, int nomor_bisnis,int flag)
	{
		double sum=0;
		String nama_produk="";
		try {
			String result = query.selectgetnamaplan(kode_bisnis, nomor_bisnis);
			if(result != null) {
				nama_produk = result.toString();		  	
			}
			if(nama_produk.contains("PLAN A")){
				idec_up = 200000;
			}else if(nama_produk.contains("PLAN B")){
				idec_up = 300000;
			}else if(nama_produk.contains("PLAN C")){
				idec_up = 400000;
			}else if(nama_produk.contains("PLAN D")){
				idec_up = 500000;
			}else if(nama_produk.contains("PLAN E")){
				idec_up = 800000;
			}else if(nama_produk.contains("PLAN F")){
				idec_up = 1000000;
			}else if(nama_produk.contains("PLAN G")){
				idec_up = 1500000;
			}else if(nama_produk.contains("PLAN H")){
				idec_up = 2000000;
			}else if(nama_produk.contains("PLAN I")){
				idec_up = 5000000;
			}else if(nama_produk.contains("PLAN J")){
				idec_up = 7500000;
			}/*else if(nama_produk.contains("PLAN K")){
				idec_up = 800000;
			}else if(nama_produk.contains("PLAN L")){
				idec_up = 900000;
			}else if(nama_produk.contains("PLAN M")){
				idec_up = 1000000;
			}else if(nama_produk.contains("PLAN N")){
				idec_up = 1500000;
			}else if(nama_produk.contains("PLAN O")){
				idec_up = 2000000;
			}else if(nama_produk.contains("PLAN P")){
				idec_up = 5000000;
			}else if(nama_produk.contains("PLAN Q")){
				idec_up = 7500000;
			}*/
		}
		catch (Exception e) {
			err=(e.toString());
		} 
		sum=idec_up ;
	
		return sum;
	}
	
	public int set_klas(int klas)
	{
		iiclass=0;
		return iiclass;
	}
	
	public int set_unit(int unit)
	{
		iiunit=0;
		return iiunit;
	}

	public String range_class(int ii_age,int ii_class)
	{
		String hsl="";
		return hsl;
	}
	
	public String range_unit(int unit)
	{
		String hsl="";
		return hsl;
	}

	public void count_rate(int klas, int unit ,int kode_produk, int nomor_produk, String kurs, int umurttg, int umurpp, double up, double premi, int pmode, int flag, int ins_period,int payperiod)
	{
		String nama_produk="";
		mbu_jumlah =0;
		mbu_rate=0;
		mbu_persen=0;
				
		if (Integer.toString(kode_produk).trim().length()==1)
		{
			nama_produk="produk_asuransi.n_prod_0"+Integer.toString(kode_produk);	
		}else{
			nama_produk="produk_asuransi.n_prod_"+Integer.toString(kode_produk);	
		}

		try{
			Class aClass2 = Class.forName( nama_produk );
			n_prod produk2 = (n_prod)aClass2.newInstance();
			produk2.setSqlMap(sqlMap);
			produk2.count_rate_848(klas,unit,848,nomor_produk,kurs,umurttg,umurpp,up,premi,pmode,flag,ins_period,payperiod);
			mbu_jumlah =  produk2.ldec_temp2;
			mbu_rate = produk2.ldec_rate2;
			mbu_persen = mbu_rate ;
		}
		catch (ClassNotFoundException e)
		{
			logger.error("ERROR :", e);
			throw new NoClassDefFoundError (e.getMessage());
		} catch (InstantiationException e) {
			logger.error("ERROR :", e);
		} catch (IllegalAccessException e) {
			logger.error("ERROR :", e);
		}		
	}		
	
	public static void main(String[] args) {
	}
}
