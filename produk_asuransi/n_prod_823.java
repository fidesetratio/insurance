 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class n_prod_823 extends n_prod{
...
//	of_set_usia_tt
	...
	public String of_check_usia_kesehatan(int utama, int hub, int tahun1, int bulan1, int tanggal1, int tahun2, int bulan2, int tanggal2, int lm_byr,int nomor_produk, int nomor_rider) 
	{
		...
		int hari=umr.hari(tahun1, bulan1, tanggal1, tahun2, bulan2, tanggal2);
		if(utama!=0){
			if(hub == 2 || hub == 5 || hub == 17 || hub == 23 || hub == 33){//suami/istri,Orang Tua
				ii_age_from = 17;
				ii_age_to = 60;
				if(ii_usia_tt < ii_age_from){
					hsl="Usia Masuk untuk EKA SEHAT PROVIDER minimum : " + ii_age_from+ " tahun";
				}	
			}else{
				ii_age_from = 0;
				ii_age_to = 24;
				if (hari < 15)
				{
					hsl="Usia Masuk untuk EKA SEHAT PROVIDER minimum 15 Hari";
				}	
			}
		}else{
			...
			if (ii_usia_tt > ii_age_to)
			{
				hsl="Usia Masuk untuk EKA SEHAT PROVIDER maximum : " + ii_age_to +" tahun";
			}
		}
		return hsl;		
	}
	...
	public double of_get_rate1( int li_class, int flag_jenis_plan, int nomor_bisnis, int umurttg, int umurpp)
	{
		...
		rate_rider=0;
		try {
			Double result = query.selectRateRider(is_kurs_id, umurttg, 0, 823, nomor_bisnis);
			if(result != null) {
	...
	{
		//utk masa asuransinya, pada produk link, eka sehat batasnya sampai 61 tahun saja.
	...
	public double hit_premi_rider(double rate,double up,double persen, double premi)
	{
		...
		if(ii_bisnis_no == 225 || ii_bisnis_no == 226 ){ //ngak ada diskon
			premi = rate * persen;
			return premi;
		}
	...
	public void count_rate(int klas, int unit ,int kode_produk, int nomor_produk, String kurs, int umurttg, int umurpp, double up, double premi, int pmode, int flag, int ins_period,int payperiod)
	{
		...
		try{
			...
			produk2.count_rate_823(klas,unit,823,nomor_produk,kurs,umurttg,umurpp,up,premi,pmode,flag,ins_period,payperiod);
