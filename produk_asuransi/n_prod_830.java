...
//n_prod_830 (Ladies Insurance – Rider)
/*
 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class n_prod_830 extends n_prod{
...
//	of_set_usia_tt
	...
	public String of_check_usia_rider(int tahun1, int bulan1, int tanggal1,int tahun2, int bulan2, int tanggal2,int lm_byr,int nomor_produk, int nomor_rider) 
	{
		...
		{
			hsl="Usia Masuk Rider Ladies Insurance ini minimum : " + ii_age_from +" tahun";
		}
		if (ii_usia_tt > ii_age_to)
		{
			hsl="Usia Masuk Rider Ladies Insurance ini maximum : " + ii_age_to+ " tahun";
		}
	...
	public double of_get_rate1( int li_class, int flag_jenis_plan, int nomor_bisnis, int umurttg, int umurpp)
	{
		...
		try {
			if( nomor_bisnis==1){
				 result = query.selectRateRider(is_kurs_id, umurttg, 0, 830, 1);
			}else if( nomor_bisnis==2){
				 result = query.selectRateRider(is_kurs_id, umurttg, 0, 830, 2);
			}else{
				result = query.selectRateRider(is_kurs_id, umurttg, 0, 830, 3);
			}
	...
	public void wf_set_premi(int tahun,int bulan,int tanggal, int cara_bayar,int tahun_1,int bulan_1,int tanggal_1,int insperiod, int flag_jenis_plan, int ii_age, int lama_bayar, int flag_cerdas_siswa, int umurpp,int kode_bisnis,int number_bisnis)
	{
		...
		if (flag_cerdas_siswa==1)
		{
			li_insured = li_sd - umurpp;
		}
...
//	*Untuk mendapatkan UP Rider berdasarkan Persentase UP
	...
	public void count_rate(int klas, int unit ,int kode_produk, int nomor_produk, String kurs, int umurttg, int umurpp, double upForCI, double premi, int pmode, int flag, int ins_period,int payperiod)//FIXIN
	{
		...
		try{
			...
			produk2.count_rate_830(klas, unit,830, nomor_produk, kurs, umurttg, umurpp, upForCI, premi,pmode, flag, ins_period, payperiod);// FIXIN , persenUp second2
