...
//n_prod_827 Waiver 55 TPD/CI – Waiver of Premium Rider
/*
...
public class n_prod_827 extends n_prod{
...
//			percentage=(rate * (up* factor_x) / 1000) * persen; //UP rider=up*0.5
	...
	public String of_check_usia_rider(int tahun1, int bulan1, int tanggal1,int tahun2, int bulan2, int tanggal2,int lm_byr,int nomor_produk, int nomor_rider) 
	{
		...
		if (nomor_rider ==1 ){
			...
			{
			hsl="Usia Masuk Rider Waiver 55 TPD/CI minimum : " + ii_age_from+" tahun";
			}
			if (ii_usia_tt > (ii_age_to-ii_contract_period))
			{
				hsl="Usia Masuk Rider Waiver 55 TPD/CI maximum : " + ii_age_to+" tahun";
			}
		}
		if (nomor_rider ==2 ){
			ii_age_to = 60;
			if (ii_usia_tt  < ii_age_from)
			{
			hsl="Usia Masuk Rider Waiver 60 TPD/CI minimum : " + ii_age_from+" tahun";
			}
			if (ii_usia_tt > (ii_age_to-ii_contract_period))
			{
				hsl="Usia Masuk Rider Waiver 60 TPD/CI maximum : " + ii_age_to+" tahun";
			}
		}
		if (nomor_rider ==3 ){
			ii_age_to = 65;
			if (ii_usia_tt  < ii_age_from)
			{
			hsl="Usia Masuk Rider Waiver 65 TPD/CI minimum : " + ii_age_from+" tahun";
			}
			if (ii_usia_tt > (ii_age_to-ii_contract_period))
			{
				hsl="Usia Masuk Rider Waiver 65 TPD/CI maximum : " + ii_age_to+" tahun";
			}
		}
	...
	public double of_get_rate1( int li_class, int flag_jenis_plan, int nomor_bisnis, int umurttg, int umurpp)
	{
		...
		if (nomor_bisnis==4 || nomor_bisnis==5 )
		{
			usia_pp = 1;
		}
		
		try {
			Double result = query.selectRateRider(is_kurs_id, umurttg, usia_pp, ii_bisnis_id_utama, nomor_bisnis);
			if(result != null) {
	...
	public void count_rate(int klas, int unit ,int kode_produk, int nomor_produk, String kurs, int umurttg, int umurpp, double up, double premi, int pmode, int flag, int ins_period,int payperiod)
	{
		...
		try{
			...
			produk2.count_rate_827(klas,unit,827,nomor_produk,kurs,umurttg,umurpp,up,premi,pmode,flag,ins_period,payperiod);
