...
//n_prod_816 (WAIVER 60 CI – Waiver of Premium Rider)
/*
 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class n_prod_814 extends n_prod{
...
//	of_set_usia_tt
	...
	public double of_get_rate1( int li_class, int flag_jenis_plan, int nomor_bisnis, int umurttg, int umurpp)
	{
		...
		try {
			Double result = query.selectRateRider(is_kurs_id, umurttg, 0, 814, 1);
			if(nomor_bisnis==1){
				result = query.selectRateRider(is_kurs_id, umurttg, 0, 814, nomor_bisnis);
			}else{
				if(nomor_bisnis==4 || nomor_bisnis==5){
					result = query.selectRateRider(is_kurs_id, umurttg, 1, 814, nomor_bisnis);
				}else{
					result = query.selectRateRider(is_kurs_id, umurttg, 0, 814, nomor_bisnis);
				}
			}
	...
	public void count_rate(int klas, int unit ,int kode_produk, int nomor_produk, String kurs, int umurttg, int umurpp, double up, double premi, int pmode, int flag, int ins_period,int payperiod)
	{
		...
		try{
			...
			produk2.count_rate_814(klas,unit,814,nomor_produk,kurs,umurttg,umurpp,up,premi,pmode,flag,ins_period,payperiod);
			mbu_jumlah =  produk2.ldec_temp5;
			mbu_rate = produk2.ldec_rate5;
