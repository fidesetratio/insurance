...
//n_prod_813 (CI Additional – Critical Illness Rider)
/*
 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class n_prod_813 extends n_prod{
	...
	public n_prod_813()
	{
		...
		ii_age_from = 1; // (11/02/2013) ganti dari 16 jadi 1
		ii_age_to = 60;		
...
//	of_set_usia_tt
	...
	public double of_get_rate1( int li_class, int flag_jenis_plan, int nomor_bisnis, int umurttg, int umurpp)
	{
		...
		rate_rider=0;
		try {
			Double result = query.selectRateRider(is_kurs_id, umurttg, 0, 813, 8);
			if(result != null) {
...
//	*Untuk mendapatkan UP Rider berdasarkan Persentase UP
	...
	public void count_rate(int klas, int unit ,int kode_produk, int nomor_produk, String kurs, int umurttg, int umurpp, double upForCI, double premi, int pmode, int flag, int ins_period,int payperiod)//FIXIN
	{
		...
		try{
			...
			produk2.count_rate_813(klas, unit,813, nomor_produk, kurs, umurttg, umurpp, upForCI, premi,pmode, flag, ins_period, payperiod);// FIXIN , persenUp second2
