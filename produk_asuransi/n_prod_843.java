 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class n_prod_843 extends n_prod{
...
//	of_set_usia_tt
	...
	public String of_check_usia_kesehatan(int utama, int hub, int tahun1, int bulan1, int tanggal1,int tahun2, int bulan2, int tanggal2,int lm_byr,int nomor_produk, int nomor_rider) 
	{
		...
		if(hub != 1 && hub != 5 && hub != 2 && hub != 17 && hub != 4 && hub != 8 && hub != 21){
			hsl="Untuk Produk PK Provestara hubungan tertanggung hanya bisa istri/suami , Orang tua, anak-anak yang sah (termasuk adopsi) dalam keluarga inti pemegang polis";
		}
		if(hub != 4 && hub != 8 && hub != 21 && hub != 22){
			...
			if(ii_usia_tt < ii_age_from){
				hsl="Usia Masuk untuk Suami/Istri dan Orang Tua SMiLe PK Provestara minimum : " + ii_age_from+ " tahun";
			}else if (ii_usia_tt > ii_age_to){
				hsl="Usia Masuk untuk Suami/Istri dan Orang Tua SMiLe PK Provestara maximum : " + ii_age_to +" tahun";
			}
		}else{
			ii_age_from = 0;
			if(utama == 0){
			...
			if(ii_usia_tt < ii_age_from){
				hsl="Usia Masuk untuk SMiLe PK Provestara minimum : " + ii_age_from+ " tahun";
			}else if (ii_usia_tt > ii_age_to){
				hsl="Usia Masuk untuk SMiLe PK Provestara maximum : " + ii_age_to +" tahun";
			}
		}
		if (hari < 15)
		{
			hsl="Usia Masuk untuk SMiLe PK Provestara minimum 15 Hari";
		}		
	...
	public double of_get_rate1( int li_class, int flag_jenis_plan, int nomor_bisnis, int umurttg, int umurpp)
	{
		...
		rate_rider=0;
		try {
			Double result = query.selectRateRider(is_kurs_id, umurttg, 0, 843, nomor_bisnis);
			if(result != null) {
	...
	public void count_rate(int klas, int unit ,int kode_produk, int nomor_produk, String kurs, int umurttg, int umurpp, double up, double premi, int pmode, int flag, int ins_period,int payperiod)
	{
		...
		try{
			...
			produk2.count_rate_839(klas,unit,843,nomor_produk,kurs,umurttg,umurpp,up,premi,pmode,flag,ins_period,payperiod);
	...
	public String of_check_usia_rider(int tahun1, int bulan1, int tanggal1,int tahun2, int bulan2, int tanggal2,int lm_byr,int nomor_produk, int nomor_rider) 
	{
		...
		if (ii_usia_tt < ii_age_from)
		{
			hsl="Usia Masuk Plan PK Provestara minimum : " + ii_age_from;
		}

		if (ii_usia_tt > ii_age_to)
		{
			hsl="Usia Masuk Plan PK Provestara maximum : " + ii_age_to;
		}
