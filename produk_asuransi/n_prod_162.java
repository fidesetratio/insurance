 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class n_prod_162 extends n_prod{

	public n_prod_162()
	{
		ii_bisnis_id = 162;
		ii_contract_period = 88;
		ii_age_from = 1;
		ii_age_to = 65;
	...
	public double of_get_min_up()
	{
		...
		if (ii_bisnis_no <=2)
		{
			...
			if (is_kurs_id.equalsIgnoreCase("01") )
			{
				if (ii_pmode==0)
				{
					ldec_1 = 250000; //sekaligus
				}else{
					if (ii_pmode==3)
					{
						ldec_1 =75000; //tahunan
					}else{
						if (ii_pmode==1)
						{
							ldec_1 = 20000;//triwulan
						}else{
							if (ii_pmode==2)
							{
								ldec_1 = 40000 ;//semesteran
							}else{
								if (ii_pmode==6)
								{
									ldec_1 = 10000 ; //bulanan
								}							
							}
						}
					}
	...
	*/
	...
	public String cek_max_up(Integer li_umur_ttg, Integer kode_produk, Double premi, Double up, Double fltpersen, Integer pmode_id,String kurs )
	{
		...
		if( pmode_id.intValue() != 0)
		{
			int faktor = 0;
			try {
				Integer result = query.faktorexcell80(li_umur_ttg.intValue(),kode_produk.intValue());
				if (result!=null)
				{		
		...
		if(kurs.equalsIgnoreCase("01"))
		{
			switch (pmode_id.intValue())
			{
				...
				case 1:
					...
					{
						hsl="Minimum Total Premi Triwulanan (3 bulanan ) : Rp. 400.000,-";
					}
					break;
				case 2:
					if (total_premi < 800000)
					{
						hsl="Minimum Total Premi Semesteran (6 bulanan ) : Rp. 800.000,-";
					}
					break;
				case 3:
				...
				case 6:
					...
					{
						hsl="Minimum Total Premi Bulanan (1 bulanan) : Rp. 200.000,-";
					}
					break;
			}
		}else{
			switch (pmode_id.intValue())
			{
				...
				case 2:
					if (total_premi < 150)
					{
						hsl="Minimum Total Premi Semesteran (6 bulanan ) :US$   150,-";
					}
					break;
				case 3:
					if (total_premi < 300)
