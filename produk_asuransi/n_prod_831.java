...
//n_prod_831 (Ladies HCP – Hospital Cash Plan Rider)
/*
 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class n_prod_831 extends n_prod{
...
//	of_set_usia_tt
	...
	public String of_check_usia_rider(int tahun1, int bulan1, int tanggal1,int tahun2, int bulan2, int tanggal2,int lm_byr,int nomor_produk, int nomor_rider) 
	{
		...
		if (bln < 12)
		{
			hsl="Usia Masuk untuk Ladies HCP minimum : " + ii_age_from+ " tahun";
		}		
		
		int x = ii_age_to -  lm_byr; 

		if ((ii_usia_tt > ii_age_to) && (ii_usia_tt > x)) 
		{
			hsl="Usia Masuk untuk Ladies HCP maximum : " + ii_age_to +" tahun";
		}
		return hsl;		
	}	
	...
	public double of_get_rate1( int li_class, int flag_jenis_plan, int nomor_bisnis, int umurttg, int umurpp)
	{
		...
		rate_rider=0;
		try {
			Double result = query.selectRateRider(is_kurs_id, umurttg, 0, 831, nomor_bisnis);
			if(result != null) {
	...
	public void count_rate(int klas, int unit ,int kode_produk, int nomor_produk, String kurs, int umurttg, int umurpp, double up, double premi, int pmode, int flag, int ins_period,int payperiod)
	{
		...
		try{
			...
			produk2.count_rate_831(klas,unit,831,nomor_produk,kurs,umurttg,umurpp,up,premi,pmode,flag,ins_period,payperiod);
